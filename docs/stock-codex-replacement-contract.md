...
install/provisioning and guided auth-onboarding gates. Direct clean-Mac targets
can use the same aggregate through the release wrapper when the operator prepares
a disposable SSH account with a real `gui/<uid>` Aqua session for the
...
automated browser/device login UX, token freshness validation, Keychain-managed
credential UX, and broader UI/device bridge coverage remain separate decisions.
