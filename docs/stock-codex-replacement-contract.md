...
runtime. The next canonical gate is signed/notarized `.pkg` plus Gatekeeper
validation.
...
guided onboarding from the installed runtime; signed/notarized package
validation is the next canonical package gate.
...
9. `stock-codex-compat` bridge proof
   ...
   - Remaining product decision: decide whether compatibility mode may require
     ...
     classifier output.
     The remaining carry-parity gates now follow the canonical production
     sequence: signed/notarized package validation, broader bridge coverage
     such as XcodeBuildMCP tests or UI automation if product scope requires it,
     and diagnostics.
