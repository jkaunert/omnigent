...
`xcrun`, `spctl`, `notarytool`, and `stapler`, but no Developer ID Installer
identity and no notarytool profile. The missing inputs are
`OMNIGENT_PKG_SIGN_IDENTITY` or `--pkg-sign-identity`, and
`OMNIGENT_NOTARYTOOL_PROFILE` or `--notarytool-profile`. This turns the next
production step into credential-backed execution of the existing gate, not more
unsigned packaging mechanics.
...
and `stapler`), but no Developer ID Installer identity is installed or selected
and no notarytool keychain profile is configured. The proof therefore did not
build a signed artifact, submit to Apple, staple, or run Gatekeeper assessment;
those are ready to execute once `OMNIGENT_PKG_SIGN_IDENTITY` or
`--pkg-sign-identity` and `OMNIGENT_NOTARYTOOL_PROFILE` or
`--notarytool-profile` are supplied.
