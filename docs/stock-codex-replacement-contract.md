...
`--clean-vm-remote-codex-home <remote-absolute-path>`, requires that path to
already contain `auth.json`, installs and provisions the signed package from
the official channel, runs a live model turn through the installed compatibility
launcher with that remote `CODEX_HOME`, verifies the auth remains available
after the turn, then removes Omnigent/package state while leaving the
...
a fail-closed verifier gate until the operator intentionally runs login there.
...
install/provisioning, guided auth-onboarding, and no-host-auth-upload
existing-auth verifier gates. Direct clean-Mac targets can use the same aggregate
through the release wrapper when the operator prepares a disposable SSH account
with a real `gui/<uid>` Aqua session for the
...
automated browser/device login UX, remote login-home preparation, token
