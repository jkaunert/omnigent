...
browser/device login, post-login token persistence, keychain behavior, and
account refresh semantics as separate product gates.
...
the installed compatibility launcher, and signed/notarized `.pkg` distribution.
...
automated browser/device login UX, and broader UI/device bridge coverage remain
separate decisions.
