...
low-privilege Apple-docs and XcodeBuildMCP build/run execution through the
packaged file bridge, plus clean user-cache stock-Codex provisioning from the
pkg-installed runtime and clean auth onboarding from the pkg-installed runtime.
...
login UX, and broader bridge coverage remain separate decisions.
...
9. `stock-codex-compat` bridge proof
   ...
   - Remaining product decision: decide whether compatibility mode may require
     ...
     coverage such as XcodeBuildMCP tests or UI automation if product scope
     requires it, and diagnostics.
