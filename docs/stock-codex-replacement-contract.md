...
user-cache stock-Codex provisioning from the pkg-installed runtime and clean
auth onboarding from the pkg-installed runtime, and signed/notarized `.pkg`
distribution.
...
9. `stock-codex-compat` bridge proof
   ...
   - Remaining product decision: decide whether compatibility mode may require
     ...
     the wrapper file bridge if product scope requires it, plus any production
     update/channel policy choices.
