...
pkg-installed runtime, and signed/notarized `.pkg` distribution.
...
9. `stock-codex-compat` bridge proof
   ...
   - Remaining product decision: decide whether compatibility mode may require
     ...
     acceptance. The remaining carry-parity gates now focus on broader bridge
     coverage such as screenshot, snapshot, gesture, or device execution through
     the wrapper file bridge if product scope requires it, diagnostics, and any
     production update/channel policy choices.
