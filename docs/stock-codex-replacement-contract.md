...
root to install this package.` The proof now supports a two-phase production
path: run `stock-codex-compat-pkg-signed-notarized --pkg-output-path
...
installer prerequisite. A live split run produced
...
`Accepted`, reported final stapled package SHA-256
`68f71a7a20d4f8059cd8f7a812096d045dbdab1fa7baf07df50f11dd86687fe3`, and the
prebuilt lifecycle consumer saw the same SHA while validating Gatekeeper source
`Notarized Developer ID` before the root block. The proof path is covered at
the harness layer:
...
doctor, payload cleanup, receipt forget, and image detach all have focused
tests. The remaining live validation is an admin-authenticated package-manager
run using the prebuilt artifact.
...
9. `stock-codex-compat` bridge proof
   ...
   - Remaining product decision: decide whether compatibility mode may require
     ...
     The live install remains blocked in the current agent process by macOS
     admin requirements for `/usr/sbin/installer`; rerun the consumer from an
