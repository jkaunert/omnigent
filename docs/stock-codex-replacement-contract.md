...
operator-provided auth home intact. The current target
`omnigent-clean@10.0.0.10` has no authenticated remote auth home yet, so this is
a fail-closed verifier gate until the operator-assisted auth-login proof
completes there.
...
Tart VM name. It expects the operator to prepare and mark the disposable account
...
uvx --from . python scripts/prove_stock_codex_replacement.py \
  ...
  --clean-vm-ssh-target omnigent-clean@10.0.0.10 \
  --clean-vm-ssh-identity ~/.ssh/mba_github_ssh_key \
  --clean-vm-remote-codex-home /Users/omnigent-clean/.codex-omnigent-existing-auth \
  --live-proof-timeout 600
```
