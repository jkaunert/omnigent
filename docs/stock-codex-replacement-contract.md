...
| Signed/notarized stock Codex compatibility pkg clean-VM automation | `blocked` until a disposable SSH/Tart macOS VM target exists | `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-clean-vm --codex-path /Users/joshuakaunert/.local/omnigent/codex-stock/0.142.5/codex --pkg-path <artifact.pkg>` adds the clean-VM automation surface. It accepts either `--clean-vm-ssh-target <user@host>` or `--clean-vm-tart-name <name> --clean-vm-ssh-user <user>`, transfers the signed/notarized package plus a file-backed stock Codex channel artifact into the VM, requires the VM user's home marker `.omnigent-stock-codex-compat-clean-user-ok`, refuses preexisting user/package state, uses noninteractive `sudo -n` to install the package into the VM's real `/Library`, runs the installed runtime through `uvx` to provision stock Codex, install the adapter package and compatibility launcher, verify PATH selection, delegate `--version`, run doctor, classify clean auth as `needs-auth`, uninstall the user launcher, remove created user state, remove `/Library/Application Support/Omnigent/stock-codex-compat`, forget the receipt, and remove the remote work directory. Unit coverage in `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_clean_vm_requires_target`, `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_clean_vm_blocks_missing_tart_vm`, and `tests/scripts/test_prove_stock_codex_replacement.py::test_clean_vm_remote_script_requires_marker_and_noninteractive_sudo` covers fail-closed target selection, missing Tart VM classification, marker enforcement, noninteractive sudo shape, and the remote proof script contract. On this host, Tart `2.32.1` and SSH/SCP are present, but `tart list` is empty, so the live clean-VM proof is correctly blocked until a disposable VM is created or supplied. This does not yet prove the live VM run, remote stock-Codex GitHub acquisition from inside the VM, automated first-boot VM setup, or browser/device auth onboarding. |
...
real `/Library` package install plus VM-user bootstrap, auth classification,
rollback, payload cleanup, and receipt cleanup. It requires the same disposable
home marker and noninteractive `sudo -n` inside the VM, so it blocks cleanly for
ordinary desktop VMs or unprepared CI images. Current host preflight found Tart
`2.32.1`, SSH, and SCP, but `tart list` is empty; live clean-VM evidence remains
pending until a disposable VM is supplied or created.
