...
| Signed/notarized stock Codex compatibility macOS pkg | `replacement-ready` for Developer ID signed, notarized, stapled, and Gatekeeper-accepted `.pkg` distribution | `scripts/build_stock_codex_compat_pkg.py` accepts an optional Developer ID Installer signing identity and keychain and builds signed packages with `pkgbuild --sign ... --timestamp` while preserving the unsigned package gate. `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-signed-notarized --notarytool-profile OmnigentExperiment` classifies signing/notary prerequisites, builds the signed package, validates `pkgutil --check-signature`, submits it with `xcrun notarytool submit --output-format json`, waits on the returned submission id with `xcrun notarytool wait`, staples it with `xcrun stapler staple`, validates with `xcrun stapler validate`, and runs `spctl -a -vv -t install`. On the credential-backed rerun, the proof reported `status=replacement-ready`, autodiscovered `Developer ID Installer: Joshua Kaunert (HSRQC9N69B)`, used notarytool profile `OmnigentExperiment`, found one Developer ID Installer identity and one Developer ID Application identity, built package identifier `ai.omnigent.stock-codex-compat` version `0.3.0.dev0`, reported package SHA-256 `d5cbd4d2458847d7380139b6562ee75f83eb7bfbde884cfcf4dc4b48085b6c16` and source bundle SHA-256 `b4eddd2748df4608e01811c1c35fdbed78af72e45993b130be060d01da1b1a07`, verified `signature_status=signed by a developer certificate issued by Apple for distribution`, received notary submission `58d38d11-347b-4d2c-82e1-e5c6d1ef6a6b` with status `Accepted`, stapled successfully, validated successfully, and `spctl` accepted the package with source `Notarized Developer ID` and origin `Developer ID Installer: Joshua Kaunert (HSRQC9N69B)`. Unit coverage in `tests/scripts/test_build_stock_codex_compat_pkg.py::test_pkgbuild_command_includes_developer_id_signing_args`, `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_signed_notarized_blocks_when_prereqs_missing`, `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_signing_prereqs_explain_application_identity_mismatch`, and `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_signed_notarized_runs_distribution_checks` covers signing argv, blocked-prereq classification, Application-vs-Installer mismatch reporting, and the notarize/staple/Gatekeeper command sequence without requiring real credentials. This closes the signed/notarized `.pkg` distribution gate for the current host; it does not perform a persistent `/Library` install, LaunchServices registration, app-bundle distribution, update scheduling, or clean-machine live model turn from the installed package. |
| Signed/notarized stock Codex compatibility pkg installer lifecycle | `blocked` for live package-manager install on this non-root agent host; unit-covered replacement-ready path | `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-installer-lifecycle --codex-path /Users/joshuakaunert/.local/omnigent/codex-stock/0.142.5/codex --notarytool-profile OmnigentExperiment` now preflights root/admin installer privileges before building or notarizing. The live Codex agent process has no passwordless sudo, so the proof reports `status=blocked` with `installer lifecycle requires root privileges for /usr/sbin/installer; run from an admin-authenticated root shell` and exits before submitting another package to Apple. The proof code builds the same signed/notarized/stapled/Gatekeeper-accepted package, creates a temporary HFS+ target image, installs with `/usr/sbin/installer -target <mountpoint>`, validates `pkgutil --volume <mountpoint>` receipt metadata and required payload files, runs the installed runtime's clean-home non-mutating doctor, removes the installed payload, forgets the target-volume receipt, and detaches the image. `tests/scripts/test_build_stock_codex_compat_pkg.py::test_postinstall_validates_target_volume_payload` covers the target-volume-aware `postinstall`; `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_installer_lifecycle_blocks_without_root` covers the admin-prereq block before build/notary; and `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_installer_lifecycle_uses_mounted_target` covers the mounted-target install, receipt, doctor, cleanup, and detach lifecycle with fake tools. This closes the harness shape for package-manager install cleanup, but the live package-manager proof still needs an admin-authenticated run. |
...
`OmnigentExperiment`; submitted package SHA-256
`d5cbd4d2458847d7380139b6562ee75f83eb7bfbde884cfcf4dc4b48085b6c16`; received
notary submission `58d38d11-347b-4d2c-82e1-e5c6d1ef6a6b` with status
...
`stock-codex-compat-pkg-installer-lifecycle` is root-blocked on the current
Codex agent host, not code-blocked. A live attempt reached `/usr/sbin/installer`
against a temporary mounted target volume and macOS returned `Must be run as
root to install this package.` A follow-up run now reports `status=blocked`
before build/notary work because this process has no passwordless sudo. The
proof path is still covered at the harness layer: target-volume-aware
`postinstall` validation, root-prerequisite classification before Apple
notarization, mounted-target install command shape, receipt validation,
installed-runtime doctor, payload cleanup, receipt forget, and image detach all
have focused tests. The remaining live validation is an admin-authenticated
package-manager run.
...
`58d38d11-347b-4d2c-82e1-e5c6d1ef6a6b`, stapled successfully, validated the
...
9. `stock-codex-compat` bridge proof
   ...
   - Remaining product decision: decide whether compatibility mode may require
     ...
     acceptance. The package-manager installer lifecycle gate has a covered
     mounted-target proof path but is live-blocked in the current agent process
     by macOS admin requirements for `/usr/sbin/installer`; rerun it from an
     admin-authenticated root context to close the live install/receipt cleanup
     gap. The production stock-Codex channel policy gate is green for
