...
| Immutable stock-Codex compatibility release promotion | `replacement-ready` for the first commit-bound direct clean-Mac package/evidence/manifest transaction | `scripts/promote_stock_codex_compat_release.py` promoted clean pushed source commit `985c7295f347caafd238cfc4c036bff3615e7310` into immutable directory `omnigent-stock-codex-compat-0.3.0.dev0-985c7295f347`. The live command used explicit signer `Developer ID Installer: Joshua Kaunert (HSRQC9N69B)` and notary profile `OmnigentExperiment`, produced package SHA-256 `adae5f4359fd66f62354de6a57fc8284a0e46771d5ba850b21729ab93e296302`, source-bundle SHA-256 `b186fb1d344301258e30cf2f00e6e12129f363ff06b8a1fb1959bf124ce5dac9`, accepted notary submission `6563c3a9-57db-475b-a329-959a7427376d`, release-evidence SHA-256 `9b08ae3e783e24687a765af36db68d29375ab8342fdee251d8526f286817d36a`, and promotion-manifest SHA-256 `0d006703bd75fb67c26d7f3eed70c263efdc45cbc1993da1fc54c7256b3ed26d`. All five direct-target steps selected official stock Codex `0.144.1` and reported `replacement-ready`; no host stock Codex or auth was uploaded. A standalone `--verify-only` rerun passed, and a final direct-target preflight proved no receipt, payload, launcher, manifest, adapter root, stock cache, or LaunchAgent residue while revalidating signature, staple, and Gatekeeper acceptance. Focused adversarial coverage includes partial-output cleanup, local/remote provenance rejection, tracked-tool binding, exact signer checks, symlink/tamper rejection, embedded metadata drift, and relocated-directory verification. That historical development-version promotion did not itself publish artifacts or finalize a stable release; the public `0.1.2` row below records those later steps. Rollout/feed policy remains separate. |
| Public stock-Codex compatibility release channel | `replacement-ready` for stable versioning, controlled GitHub publication, unauthenticated public asset verification, and clean-target URL installation | Release [`stock-codex-compat-v0.1.2`](https://github.com/jkaunert/omnigent/releases/tag/stock-codex-compat-v0.1.2) was published on 2026-07-10 from clean pushed source commit `cb2520a9e748ccf5da987926ae6a70ad519994b0`. `packaging/stock-codex-compat/VERSION` owns installer version `0.1.2` independently from Omnigent's lockstep PyPI version. The release contains signed/notarized package SHA-256 `42d61a2e93b384297d968bd1092d08eb26e4f5f62688a3c7b8908d063a97525b`, release-evidence SHA-256 `93ac1110a057fcaa64e1c5d6d2c6cbb1df195e7b7c6eaefa1bd0816021109503`, promotion-manifest SHA-256 `7d1675a5c9275b273782a63e4277d075eba5005c9df7922848fb74aa946f6713`, `SHA256SUMS`, and publication-record SHA-256 `3f0dd33395c0ab295ae628ecb0a8b6345ab220df9891888a0df38a99265a865e`; the release body anchors the publication-record digest. Accepted notarization submission `5ed4b9f2-150d-4b9b-88f4-ab0a190b1b18` and the five-step release evidence select official stock Codex `0.144.1`, with every step `replacement-ready` and no host stock Codex or auth upload. The publisher verified the draft asset set and hashes, published it, then re-read the public API and downloaded every asset without credentials. A separate `--verify-only` run passed. `scripts/prove_stock_codex_compat_published_release.py` then sent only a shell program to the marked disposable Tahoe `26.5.1` arm64 Mac, downloaded the package from the public GitHub URL, verified SHA-256/signature/staple/Gatekeeper, installed and checked receipt/payload version `0.1.2`, removed the payload and receipt, and confirmed cleanup with `published_release_package_uploaded=False`, `published_release_auth_uploaded=False`, and `published_release_cleanup=complete`. Adversarial coverage includes tag namespace, draft temporary-URL normalization and token binding, release-body digest anchoring, asset-set/hash mismatch, failed-draft cleanup, no-package-upload command shape, cleanup-marker enforcement, and uniquely correlated recovery from a post-upload `notarytool` crash. This closes the trusted-publication and clean-machine public-acquisition production gap; rollout/feed policy remains a separate product decision. |
...
uv run python scripts/publish_stock_codex_compat_release.py \
  --promotion-dir /absolute/path/to/promoted-0.1.2 \
  --output-dir /absolute/path/to/publication-0.1.2 \
  --repository jkaunert/omnigent \
  --tag stock-codex-compat-v0.1.2

uv run python scripts/publish_stock_codex_compat_release.py \
  --verify-only /absolute/path/to/publication-0.1.2/publication-record.json

uv run python scripts/prove_stock_codex_compat_published_release.py \
  --publication-record /absolute/path/to/publication-0.1.2/publication-record.json \
...
not from the development host. The real `0.1.2` run passed publication,
independent public verification, direct clean-Mac installation, receipt and
payload inspection, rollback cleanup, and final absence checks, so this gate is
`replacement-ready`.
