...
| Signed/notarized stock Codex compatibility macOS pkg | `blocked` at local signing/notary prerequisites, with harness classification in place | `scripts/build_stock_codex_compat_pkg.py` now accepts an optional Developer ID Installer signing identity and keychain and builds signed packages with `pkgbuild --sign ... --timestamp` while preserving the unsigned package gate. `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-signed-notarized` classifies signing/notary prerequisites before doing notarization work, then, when ready, builds the signed package, validates `pkgutil --check-signature`, submits it with `xcrun notarytool submit --wait --output-format json`, staples it with `xcrun stapler staple`, validates with `xcrun stapler validate`, and runs `spctl -a -vv -t install`. On the current host the proof exited successfully with `status=blocked`, found `pkgbuild`, `pkgutil`, `xcrun`, `spctl`, `notarytool`, and `stapler`, found one Developer ID Application identity, but found zero Developer ID Installer identities and no notarytool profile. The reported blocker is now explicit: Developer ID Application identities cannot sign installer packages; this `.pkg` gate requires a Developer ID Installer identity plus `OMNIGENT_NOTARYTOOL_PROFILE` or `--notarytool-profile`. Unit coverage in `tests/scripts/test_build_stock_codex_compat_pkg.py::test_pkgbuild_command_includes_developer_id_signing_args`, `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_signed_notarized_blocks_when_prereqs_missing`, `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_signing_prereqs_explain_application_identity_mismatch`, and `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_signed_notarized_runs_distribution_checks` covers signing argv, blocked-prereq classification, Application-vs-Installer mismatch reporting, and the notarize/staple/Gatekeeper command sequence without requiring real credentials. This adds the production gate shape and proves the current block is credential/setup, not package-runtime drift; it does not produce a signed artifact until Developer ID Installer and notary credentials are configured. |
...
2026-07-06 signed/notarized package gate note:
`stock-codex-compat-pkg-signed-notarized` is implemented and currently blocked
on local signing/notary prerequisites. The proof verifies prerequisite state
before doing notarization work, and on this host found `pkgbuild`, `pkgutil`,
`xcrun`, `spctl`, `notarytool`, and `stapler`. It also found one Developer ID
Application identity but zero Developer ID Installer identities, so the
available signing material is for app bundles, not installer packages. The
missing inputs are a Developer ID Installer certificate selected through
`OMNIGENT_PKG_SIGN_IDENTITY` or `--pkg-sign-identity`, and a saved notarytool
profile selected through `OMNIGENT_NOTARYTOOL_PROFILE` or
`--notarytool-profile`. The local `notarytool store-credentials` help confirms
that a profile can be created without embedding a password in shell history by
omitting `--password` and using the secure prompt:

```sh
xcrun notarytool store-credentials omnigent-notary \
  --apple-id "$APPLE_ID" \
  --team-id "$APPLE_TEAM_ID"
```

After importing or creating a Developer ID Installer certificate, rerun:

```sh
OMNIGENT_PKG_SIGN_IDENTITY="Developer ID Installer: ..." \
OMNIGENT_NOTARYTOOL_PROFILE="omnigent-notary" \
uvx --from . python scripts/prove_stock_codex_replacement.py \
  --proof stock-codex-compat-pkg-signed-notarized
```

This turns the next production step into credential-backed execution of the
existing gate, not more unsigned packaging mechanics.
...
execution through the packaged file bridge, plus clean user-cache stock-Codex
provisioning from the pkg-installed runtime and clean auth onboarding from the
pkg-installed runtime.
Signed/notarized distribution, persistent app-bundle installation,
LaunchServices/Dock/Finder default behavior, persistent update scheduling,
remote official stock-Codex acquisition/update policy, automated browser/device
...
provisioning from the installed runtime.
...
Current signed/notarized package status on 2026-07-06:
`--proof stock-codex-compat-pkg-signed-notarized` is implemented and ran to a
blocked prerequisite classification. It verified the local packaging/notary
toolchain is present (`pkgbuild`, `pkgutil`, `xcrun`, `spctl`, `notarytool`,
and `stapler`), one Developer ID Application identity is installed, but no
Developer ID Installer identity is installed or selected and no notarytool
keychain profile is configured. The proof therefore did not build a signed
artifact, submit to Apple, staple, or run Gatekeeper assessment; those are ready
to execute once a Developer ID Installer certificate is available through
`OMNIGENT_PKG_SIGN_IDENTITY` or `--pkg-sign-identity` and
`OMNIGENT_NOTARYTOOL_PROFILE` or `--notarytool-profile` is supplied.
...
9. `stock-codex-compat` bridge proof
   ...
   - Remaining product decision: decide whether compatibility mode may require
     ...
     classifier output.
     The signed/notarized package gate is now implemented and blocked only on
     local Developer ID Installer plus notarytool credentials. The remaining
     carry-parity gates now follow the canonical production sequence:
     credential-backed signed/notarized package execution, broader bridge
     coverage such as screenshot, snapshot, gesture, or device execution
     through the wrapper file bridge if product scope requires it, and
     diagnostics.
