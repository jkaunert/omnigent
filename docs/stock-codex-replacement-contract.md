...
| Pkg-installed runtime scheduled stock Codex updater | `replacement-ready` for user LaunchAgent plist generation plus direct updater execution from the installed runtime | `scripts/update_stock_codex_compat.py` is now included in the portable bundle and macOS `.pkg` runtime payload, recorded as `stockCodexUpdater` in `bundle-manifest.json` and `pkg-manifest.json`, and validated by package `postinstall`. `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-update-agent --codex-path <stock-codex>` builds the unsigned `.pkg`, expands and stages it under a temporary installed-root shape, verifies the updater payload and manifest contract, then runs the packaged updater against a clean temporary `HOME`. The updater writes a user-level LaunchAgent plist whose `ProgramArguments` invoke `uvx --from <installed-runtime> python <installed-runtime>/scripts/update_stock_codex_compat.py`, pass `--uvx-path <absolute-uvx>` so launchd's sparse PATH does not break the scheduled run, include the official channel manifest, launcher manifest, rollback metadata, and explicit remote-download permission, and intentionally omit proof-only `--current-codex`, `--write-launch-agent`, and `--run-now` flags. The same run stages the stable official stock Codex target, promotes only the clean user's launcher manifest and `OMNIGENT_STOCK_CODEX_PATH` env contract, writes rollback metadata, then a second updater run reports `up-to-date` without mutation or another remote-download flag. Unit coverage in `tests/scripts/test_update_stock_codex_compat.py` covers updater JSON, plist generation, promotion, no-op behavior, and absolute `uvx` path preservation; `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_update_agent_proof_writes_schedule_and_runs_update` covers packaged-runtime scheduling shape with fake package staging and fake stock Codex. This closes the packaged updater entrypoint and user LaunchAgent plist contract. It does not load or enable launchd state, choose LaunchAgent versus LaunchDaemon as final product policy, adopt pre-release channels, verify an independent archive signature, perform a real `/Library` install, or prove Gatekeeper behavior. |
| Pkg-installed runtime clean auth onboarding | `replacement-ready` for packaged-runtime launcher-owned auth classification and guided setup boundary | `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-clean-auth-onboarding --codex-path /Users/joshuakaunert/.local/omnigent/codex-stock/0.142.5/codex` built the unsigned `.pkg`, expanded and staged it under a temporary installed-root shape, provisioned stock Codex `0.142.5` into the clean user cache from the installed runtime's `provision_stock_codex.py`, installed the default adapter package, installed and selected `~/.local/bin/omnigent-stock-codex-compat`, verified `--version`, launcher probe, launcher manifest `repoRoot=<installed-runtime>`, and non-mutating doctor state, then ran the packaged runtime's own `omnigent.codex_native` auth classifier in subprocesses with `PYTHONPATH=<installed-runtime>`. The proof verified the current real stock auth source `/Users/joshuakaunert/.codex/auth.json` is available without printing credential material, verified a clean `CODEX_HOME` resolves `<clean-codex-home>/auth.json` and reports `needs-auth`, verified a synthetic populated `auth.json` reports available, verified the classifier used the clean-provisioned stock Codex path, and emitted a guided setup command shaped as `CODEX_HOME=<clean-codex-home> <clean-home>/.local/bin/omnigent-stock-codex-compat login`. The proof used stock Codex SHA-256 `0d9a9da26e49e62b2c36f11229920287fc05576e77140ec63621ef6fdb2ddcd1`; the proof emits package SHA-256 as run evidence but does not pin it in this row because the package includes this documentation. Unit coverage in `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_clean_auth_proof_uses_installed_runtime` covers the packaged classifier path, launcher-owned setup command, PATH selection, manifest runtime ownership, and non-mutating doctor state with fake stock Codex and auth fixtures. This closes clean auth classification and launcher-owned guided onboarding from the installed runtime. It does not automate browser/device login, validate token freshness with OpenAI, copy credentials to another machine, sign, notarize, staple, or prove Gatekeeper behavior. |
| Signed/notarized stock Codex compatibility macOS pkg | `replacement-ready` for Developer ID signed, notarized, stapled, Gatekeeper-accepted, and persistable `.pkg` distribution | `scripts/build_stock_codex_compat_pkg.py` accepts an optional Developer ID Installer signing identity and keychain and builds signed packages with `pkgbuild --sign ... --timestamp` while preserving the unsigned package gate. `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-signed-notarized --notarytool-profile OmnigentExperiment --pkg-output-path <artifact.pkg>` classifies signing/notary prerequisites, builds the signed package at the requested artifact path, validates `pkgutil --check-signature`, submits it with `xcrun notarytool submit --output-format json`, waits on the returned submission id with `xcrun notarytool wait`, staples it with `xcrun stapler staple`, validates with `xcrun stapler validate`, and runs `spctl -a -vv -t install`. On the bootstrap-backed credential rerun, the proof reported `status=replacement-ready`, autodiscovered `Developer ID Installer: Joshua Kaunert (HSRQC9N69B)`, used notarytool profile `OmnigentExperiment`, found one Developer ID Installer identity and one Developer ID Application identity, built package identifier `ai.omnigent.stock-codex-compat` version `0.3.0.dev0`, included the packaged first-run bootstrap payloads, reported final stapled package SHA-256 `7ec3bec64157f1bed9f56ad1e457ef556912ca3ddd1ac95db5b534066bdf2f3d` and source bundle SHA-256 `b358eddc4d8238844843aec8276aa181b9660f0b47a59b00a8d4d669b2377c17`, verified `signature_status=signed by a developer certificate issued by Apple for distribution`, received notary submission `7256d678-f5cc-4676-ba35-4d3db4b28820` with status `Accepted`, stapled successfully, validated successfully, and `spctl` accepted the package with source `Notarized Developer ID` and origin `Developer ID Installer: Joshua Kaunert (HSRQC9N69B)`. Unit coverage in `tests/scripts/test_build_stock_codex_compat_pkg.py::test_pkgbuild_command_includes_developer_id_signing_args`, `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_signed_notarized_blocks_when_prereqs_missing`, `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_signing_prereqs_explain_application_identity_mismatch`, and `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_signed_notarized_runs_distribution_checks` covers signing argv, blocked-prereq classification, Application-vs-Installer mismatch reporting, explicit artifact output, and the notarize/staple/Gatekeeper command sequence without requiring real credentials. This closes the user-context signed/notarized `.pkg` artifact producer; it does not perform a persistent `/Library` install, LaunchServices registration, app-bundle distribution, update scheduling, or clean-machine live model turn from the installed package. |
...
| Signed/notarized stock Codex compatibility pkg clean-VM bootstrap from raw Tart image | `replacement-ready` for cached raw Tahoe base image to prepared disposable clean-VM target, plus downstream package/update-agent handoff | `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-clean-vm-bootstrap --clean-vm-source-tart-name ghcr.io/cirruslabs/macos-tahoe-base:latest --clean-vm-tart-name omnigent-clean-bootstrap-proof --clean-vm-ssh-user admin --clean-vm-ssh-identity /Users/joshuakaunert/.ssh/mba_github_ssh_key --clean-vm-bootstrap-install-uv` cloned the cached Cirrus Labs Tahoe base image into a new target, randomized the clone MAC, started it headless, waited for Tart guest-agent readiness, added the derived SSH public key, enabled Remote Login, touched `/Users/admin/.omnigent-stock-codex-compat-clean-user-ok`, installed user-local `uvx` with shell startup-file mutation disabled, verified `sudo -n`, verified key SSH independently, and stopped the VM. Live evidence recorded `tart_clone_completed=True`, `tart_random_mac_completed=True`, `tart_exec_status=exit-0`, `ssh_ready=True`, `sudo_noninteractive=True`, `uvx_path=/Users/admin/.local/bin/uvx`, and SSH public-key SHA-256 `d702b7cd025012d411413e219bb55ba2323cca1d9f919f38282ccacfc452db0b`. The same prepared clone then passed `stock-codex-compat-pkg-clean-vm-update-agent` with signed/notarized package SHA-256 `d0185a22380036b97703144f91bbcb701806c3638ebcbe98b2aa38df43baf581`: the package installed into the clone's real `/Library`, acquired stock Codex from the official OpenAI GitHub release channel, loaded `/Users/admin/Library/LaunchAgents/ai.omnigent.stock-codex-compat.update.plist` into `gui/501`, completed `launchctl kickstart -k`, emitted `direct_action=up-to-date` and `scheduled_action=up-to-date`, proved `host_stock_codex_uploaded=False` and `host_cache_referenced=False`, removed proof-owned user/package state, and stopped the VM. Unit coverage in `tests/scripts/test_prove_stock_codex_replacement.py::test_clean_vm_bootstrap_script_installs_uvx_without_shell_profile_mutation`, `test_stock_codex_compat_pkg_clean_vm_bootstrap_blocks_existing_target`, and `test_stock_codex_compat_pkg_clean_vm_bootstrap_clones_and_verifies_ssh` covers script shape, no target overwrite, no shell-profile mutation, clone/start/guest-agent/SSH/stop choreography, and fail-closed prerequisites. This closes the raw Tart base-image preparation and handoff gap for the clean-VM package gates; it does not prove non-Tart clean-machine provisioning, automated browser/device auth login, LaunchDaemon policy, alpha/pre-release adoption, independent archive signature verification, or broader UI/device bridge coverage. |
| Signed/notarized stock Codex compatibility pkg clean-VM automation | `replacement-ready` for disposable Tart macOS VM real-`/Library` install, packaged user bootstrap, auth classification, rollback, and receipt cleanup | `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-clean-vm --codex-path /Users/joshuakaunert/.local/omnigent/codex-stock/0.142.5/codex --pkg-path /private/tmp/omnigent-stock-codex-compat-bootstrap-pkg-final.smhF6h/omnigent-stock-codex-compat.pkg --clean-vm-tart-name omnigent-clean --clean-vm-ssh-user admin --clean-vm-ssh-identity /Users/joshuakaunert/.ssh/mba_github_ssh_key` now passes against a disposable Tart Tahoe VM after VM bootstrap supplied key SSH, `uvx`, noninteractive `sudo -n`, and the clean-user marker. A fresh signed/notarized producer run created final stapled package SHA-256 `7ec3bec64157f1bed9f56ad1e457ef556912ca3ddd1ac95db5b534066bdf2f3d`, source bundle SHA-256 `b358eddc4d8238844843aec8276aa181b9660f0b47a59b00a8d4d669b2377c17`, notary submission `7256d678-f5cc-4676-ba35-4d3db4b28820`, Gatekeeper source `Notarized Developer ID`, and origin `Developer ID Installer: Joshua Kaunert (HSRQC9N69B)`. The gate transfers the signed/notarized package plus a file-backed stock Codex channel artifact into the VM, refuses preexisting user/package state, installs package `ai.omnigent.stock-codex-compat` version `0.3.0.dev0` into the VM's real `/Library`, validates the receipt and installed `runtime/scripts/bootstrap_stock_codex_compat.sh`, then calls that packaged bootstrapper. The bootstrapper stages the immutable machine runtime from `/Library/Application Support/Omnigent/stock-codex-compat/runtime` into the VM user's writable `~/.local/omnigent/stock-codex-compat/runtime`, runs `bootstrap_stock_codex_compat.py` through `uvx --from <staged-runtime>`, provisions stock Codex `codex-cli 0.142.5` with SHA-256 `0d9a9da26e49e62b2c36f11229920287fc05576e77140ec63621ef6fdb2ddcd1`, installs the default adapter package, installs and selects `~/.local/bin/omnigent-stock-codex-compat`, and runs doctor. Remote output showed `action=bootstrapped`, `repoRoot=/Users/admin/.local/omnigent/stock-codex-compat/runtime`, `existingTargetState=managed`, `targetSelectedOnPath=True`, `mutatesFilesystem=False`, clean proof-scoped auth `needs-auth`, user launcher uninstall, created user-state removal, `/Library/Application Support/Omnigent/stock-codex-compat` removal, package receipt forgetting, and remote work directory cleanup. Unit coverage in `tests/scripts/test_bootstrap_stock_codex_compat.py`, `tests/scripts/test_build_stock_codex_compat_bundle.py`, `tests/scripts/test_build_stock_codex_compat_pkg.py`, and `tests/scripts/test_prove_stock_codex_replacement.py` covers staged-runtime bootstrap sequencing, bundle/package bootstrap payload inclusion, target-volume `postinstall` checks, fail-closed target selection, missing Tart VM classification, SSH readiness waiting, explicit SSH identity selection, user-local PATH setup, noninteractive sudo shape, packaged bootstrapper invocation, proof-scoped stock Codex export, remote script contract, and non-persistent SSH known-hosts behavior. This closes the live clean-VM signed-pkg automation gate and the product-owned first-run bootstrap gap; it does not prove browser/device auth login, scheduled update agents, or a clean-machine live model turn from the installed launcher. |
| Signed/notarized stock Codex compatibility pkg clean-VM remote acquisition | `replacement-ready` for official OpenAI GitHub release acquisition inside a disposable Tart VM | `scripts/bootstrap_stock_codex_compat.py` now carries an optional `--channel-policy` through to the packaged stock Codex provisioner, and `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-clean-vm-remote-acquisition --codex-path /Users/joshuakaunert/.local/omnigent/codex-stock/0.142.5/codex --pkg-path /private/tmp/omnigent-stock-codex-compat-remote-acquisition-pkg.wFVDzP/omnigent-stock-codex-compat.pkg --clean-vm-tart-name omnigent-clean --clean-vm-ssh-user admin --clean-vm-ssh-identity /Users/joshuakaunert/.ssh/mba_github_ssh_key` passed against the disposable Tart Tahoe VM. The branch-built package was signed, notarized, stapled, and Gatekeeper accepted with package SHA-256 `9e1993bc198b83cd29e021910c0bde965668d6f6980a4eda104d9729a1f490c3`, source bundle SHA-256 `e2a65fa1e3fb285293db0d2b72e667dc8998f49f2202b31103a98cb52a48d0eb`, notary submission `2eac873b-8432-429e-9751-4edbceac60e7`, Gatekeeper source `Notarized Developer ID`, and origin `Developer ID Installer: Joshua Kaunert (HSRQC9N69B)`. The gate copied only the signed package, a URL-backed channel manifest, and the remote proof script into the VM; it did not copy the host stock Codex binary (`host_stock_codex_uploaded=False`). The VM installed the package into its real `/Library`, invoked the packaged bootstrapper with `--allow-remote-channel-download --channel-policy official-openai-github-release`, downloaded `https://github.com/openai/codex/releases/download/rust-v0.142.5/codex-aarch64-apple-darwin.tar.gz`, verified archive SHA-256 `7156b19962735c9cfb555cdd7babe8c40e7976881f8712b781199219d2e3a707`, extracted `codex-aarch64-apple-darwin`, provisioned stock Codex `codex-cli 0.142.5` into `/Users/admin/.local/omnigent/codex-stock/0.142.5/codex`, installed and selected `~/.local/bin/omnigent-stock-codex-compat`, classified clean proof-scoped auth as `needs-auth`, uninstalled the launcher, removed user state, removed `/Library/Application Support/Omnigent/stock-codex-compat`, forgot package receipt `ai.omnigent.stock-codex-compat`, and cleaned the remote work directory. Unit coverage in `tests/scripts/test_bootstrap_stock_codex_compat.py::test_bootstrap_uses_staged_runtime_to_provision_and_install`, `tests/scripts/test_prove_stock_codex_replacement.py::test_clean_vm_remote_acquisition_script_uses_url_backed_channel`, and `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_clean_vm_remote_acquisition_does_not_upload_stock_binary` covers channel-policy forwarding, URL-only channel enforcement, explicit remote download opt-in, no host binary upload, SCP plan shape, and command dispatch. This closes clean-machine stock Codex acquisition from the installed package/runtime without relying on a host-supplied stock Codex artifact; it does not prove browser/device auth login, scheduled update agents, or a clean-machine live model turn from the installed launcher. |
| Signed/notarized stock Codex compatibility pkg clean-VM auth onboarding | `replacement-ready` for installed-launcher guided login setup without credential upload or browser-login execution | `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-clean-vm-auth-onboarding --codex-path /Users/joshuakaunert/.local/omnigent/codex-stock/0.142.5/codex --pkg-path /private/tmp/omnigent-stock-codex-compat-update-agent-pkg-fixed.HhDwoi/omnigent-stock-codex-compat.pkg --clean-vm-tart-name omnigent-clean-bootstrap-proof --clean-vm-ssh-user admin --clean-vm-ssh-identity /Users/joshuakaunert/.ssh/mba_github_ssh_key --clean-vm-start-tart` passed against the prepared disposable Tahoe clone. The first attempt blocked on preexisting `/Users/admin/.local/bin/omnigent-stock-codex-compat`, confirming the clean-state guard; after removing only proof-owned stale user/package state in the disposable clone, the same gate installed the signed/notarized package into the VM's real `/Library`, acquired stock Codex from the official OpenAI GitHub release channel, selected `/Users/admin/.local/bin/omnigent-stock-codex-compat`, classified `/Users/admin/.omnigent-stock-codex-compat-clean-vm-remote-acquisition-proof/auth-onboarding-codex-home/auth.json` as `needs-auth`, and emitted `CODEX_HOME=/Users/admin/.omnigent-stock-codex-compat-clean-vm-remote-acquisition-proof/auth-onboarding-codex-home /Users/admin/.local/bin/omnigent-stock-codex-compat login`. Evidence recorded package SHA-256 `d0185a22380036b97703144f91bbcb701806c3638ebcbe98b2aa38df43baf581`, cask version `0.142.5`, official release URL `https://github.com/openai/codex/releases/download/rust-v0.142.5/codex-aarch64-apple-darwin.tar.gz`, archive SHA-256 `7156b19962735c9cfb555cdd7babe8c40e7976881f8712b781199219d2e3a707`, `host_stock_codex_uploaded=False`, `command_executed=False`, `auth_uploaded=False`, `tart_started=True`, and `tart_stopped=True`. Unit coverage in `tests/scripts/test_prove_stock_codex_replacement.py::test_clean_vm_remote_acquisition_script_uses_url_backed_channel`, `test_stock_codex_compat_pkg_clean_vm_auth_onboarding_guides_login_without_auth_upload`, and `test_clean_vm_auth_onboarding_output_evidence_requires_no_login_execution` covers remote script mode selection, no host stock binary upload, no auth upload, exact installed-launcher login command parsing, and fail-closed rejection if login execution is reported. This closes the clean-machine guided auth setup boundary; it does not prove automated browser/device login, token persistence after user login, keychain behavior, or account refresh semantics. |
| Signed/notarized stock Codex compatibility pkg clean-VM auth persistence | `replacement-ready` for proof-scoped post-login `auth.json` persistence and live installed-launcher execution | `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-clean-vm-auth-persistence --codex-path /Users/joshuakaunert/.local/omnigent/codex-stock/0.142.5/codex --pkg-path /private/tmp/omnigent-stock-codex-compat-update-agent-pkg-fixed.HhDwoi/omnigent-stock-codex-compat.pkg --clean-vm-tart-name omnigent-clean-bootstrap-proof --clean-vm-ssh-user admin --clean-vm-ssh-identity /Users/joshuakaunert/.ssh/mba_github_ssh_key --clean-vm-start-tart` extends the guided-auth clean-VM path by copying a proof-scoped stock `auth.json` into the same `auth-onboarding-codex-home` location as the stand-in for a user-completed login, then running a real model turn through `/Users/admin/.local/bin/omnigent-stock-codex-compat`. The first attempt blocked on stale proof-owned `/Library/Application Support/Omnigent/stock-codex-compat`; the proof cleanup trap removed that state and the rerun passed. Evidence recorded package SHA-256 `d0185a22380036b97703144f91bbcb701806c3638ebcbe98b2aa38df43baf581`, cask version `0.142.5`, official release URL `https://github.com/openai/codex/releases/download/rust-v0.142.5/codex-aarch64-apple-darwin.tar.gz`, `host_stock_codex_uploaded=False`, `auth_source=stock-default-home`, `auth_uploaded=True`, `pre_unavailable_reason=needs-auth`, `post_unavailable_reason=None`, `auth_persisted_after_live=True`, `login_command_executed=False`, `browser_login_automated=False`, live thread `019f42b9-db2c-71f1-8c73-95d486b6c64e`, `event_count=6`, route-prefixed agent preview containing `STOCK_CODEX_COMPAT_AUTH_PERSISTENCE_OK`, and `tart_stopped=True`. Unit coverage in `tests/scripts/test_prove_stock_codex_replacement.py::test_clean_vm_remote_acquisition_script_uses_url_backed_channel`, `test_stock_codex_compat_pkg_clean_vm_auth_persistence_uploads_auth_not_stock_binary`, and `test_clean_vm_auth_persistence_output_evidence_rejects_automated_browser_login` covers remote mode selection, proof-scoped auth upload, no host stock binary upload, exact installed-launcher auth path parsing, route-prefixed live evidence parsing, and fail-closed rejection if browser login automation is reported. This closes post-login file-backed auth persistence and live installed-launcher use from the guided `CODEX_HOME`; it does not automate browser/device login, prove Keychain-backed managed credentials, or validate long-term account refresh semantics. |
| Signed/notarized stock Codex compatibility pkg clean-VM update-agent launchd gate | `replacement-ready` for user LaunchAgent load, forced scheduled updater execution, and unload inside a disposable Tart VM | `scripts/update_stock_codex_compat.py` now includes `--uvx-path <absolute-uvx>` in generated LaunchAgent `ProgramArguments`, closing the launchd sparse-PATH failure found by the clean-VM proof. `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-clean-vm-update-agent --codex-path /Users/joshuakaunert/.local/omnigent/codex-stock/0.142.5/codex --pkg-path /private/tmp/omnigent-stock-codex-compat-update-agent-pkg-fixed.HhDwoi/omnigent-stock-codex-compat.pkg --clean-vm-tart-name omnigent-clean --clean-vm-ssh-user admin --clean-vm-ssh-identity /Users/joshuakaunert/.ssh/mba_github_ssh_key --clean-vm-start-tart` passed against the disposable Tart Tahoe VM. The fixed package was signed, notarized, stapled, and Gatekeeper accepted with package SHA-256 `d0185a22380036b97703144f91bbcb701806c3638ebcbe98b2aa38df43baf581`, source bundle SHA-256 `a8aadb3b9c18449d732e0c2330c91f0f503490ba956f353197ce59a90aa97b42`, notary submission `8e72b73c-48f6-4ad3-b063-fbc314870763`, Gatekeeper source `Notarized Developer ID`, and origin `Developer ID Installer: Joshua Kaunert (HSRQC9N69B)`. The gate copies only the signed package, a URL-backed official OpenAI GitHub release channel manifest, and the remote proof script; it does not copy the host stock Codex binary (`host_stock_codex_uploaded=False`). The VM installs the package into real `/Library`, bootstraps the staged user runtime, writes `/Users/admin/Library/LaunchAgents/ai.omnigent.stock-codex-compat.update.plist`, validates scheduled `ProgramArguments`, loads it into launch domain `gui/501` with `launchctl bootstrap`, forces one run with `launchctl kickstart -k`, parses the scheduled updater JSON from the LaunchAgent stdout log, verifies `scheduled_action=up-to-date`, unloads with `launchctl bootout`, records selected in-VM stock Codex `/Users/admin/.local/omnigent/codex-stock/0.142.5/codex`, verifies `host_cache_referenced=False`, then removes user/package state and stops the VM. Unit coverage in `tests/scripts/test_update_stock_codex_compat.py` covers absolute `uvx` path preservation in the scheduled plist, and `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_clean_vm_update_agent_loads_launchd_without_stock_binary` plus `test_clean_vm_update_agent_output_evidence_requires_launchctl_completion` cover no host-binary upload, update-agent command dispatch, launchctl evidence parsing, and fail-closed evidence requirements. This closes clean-machine launchd enablement for the scheduled updater. It does not prove automated browser/device auth login, LaunchDaemon policy, alpha/pre-release adoption, or independent archive signature verification. |
| Signed/notarized stock Codex compatibility pkg clean-VM installed-launcher live model turn | `replacement-ready` for a real model turn from the installed compatibility launcher after in-VM official stock Codex acquisition | `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-clean-vm-live --codex-path /Users/joshuakaunert/.local/omnigent/codex-stock/0.142.5/codex --pkg-path /private/tmp/omnigent-stock-codex-compat-remote-acquisition-pkg.wFVDzP/omnigent-stock-codex-compat.pkg --clean-vm-tart-name omnigent-clean --clean-vm-ssh-user admin --clean-vm-ssh-identity /Users/joshuakaunert/.ssh/mba_github_ssh_key --clean-vm-start-tart --live-proof-timeout 600` extends the clean-VM remote-acquisition proof by verifying the current real stock auth source is available, uploading only a proof-scoped copy of `/Users/joshuakaunert/.codex/auth.json`, and running the clean VM's installed `~/.local/bin/omnigent-stock-codex-compat` launcher through `exec --json` with `CODEX_HOME=<proof-root>/live-codex-home`, `OMNIGENT_STOCK_CODEX_PATH=<provisioned-codex>`, `--sandbox read-only`, and the staged user runtime as `cwd`. The live run used the same signed/notarized package SHA-256 `9e1993bc198b83cd29e021910c0bde965668d6f6980a4eda104d9729a1f490c3`, official channel policy `official-openai-github-release`, release URL `https://github.com/openai/codex/releases/download/rust-v0.142.5/codex-aarch64-apple-darwin.tar.gz`, archive SHA-256 `7156b19962735c9cfb555cdd7babe8c40e7976881f8712b781199219d2e3a707`, and did not upload the host stock Codex binary (`host_stock_codex_uploaded=False`). The proof recorded `auth_source=stock-default-home`, `auth_uploaded=True`, parsed `selectedCommandPath=launcherPath=~/.local/bin/omnigent-stock-codex-compat`, live `CODEX_HOME`, staged-runtime working directory, thread `019f3fd9-093f-7550-8474-3c67eb5fe6c9`, `event_count=6`, and first agent-message preview `Routing: orchestrator-led` followed by `apple-appdev-workflow:apple-app-orchestrator` before `STOCK_CODEX_COMPAT_LIVE_OK`; the VM was stopped after the run. Unit coverage in `tests/scripts/test_prove_stock_codex_replacement.py::test_clean_vm_remote_acquisition_script_uses_url_backed_channel`, `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_clean_vm_remote_acquisition_does_not_upload_stock_binary`, `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_clean_vm_live_uploads_auth_but_not_stock_binary`, and `tests/scripts/test_prove_stock_codex_replacement.py::test_clean_vm_live_output_evidence_requires_installed_launcher_surface` covers optional auth upload, URL-only channel enforcement, no host binary upload, live thread evidence parsing, installed-launcher surface enforcement, SCP plan shape, and live proof command dispatch. This closes the clean-machine live model turn from the installed launcher without relying on a host-supplied stock Codex artifact; it does not prove automated browser/device auth login or persistent update promotion. |
| Signed/notarized stock Codex compatibility pkg direct clean-Mac release aggregate | `replacement-ready` for the five-step release aggregate on an operator-supplied non-Tart macOS target | `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-clean-vm-release --codex-path /Users/joshuakaunert/.local/omnigent/codex-stock/0.142.5/codex --pkg-path /Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.pkg --clean-vm-ssh-target omnigent-clean@10.0.0.10 --clean-vm-ssh-identity /Users/joshuakaunert/.ssh/mba_github_ssh_key --live-proof-timeout 600` passed on 2026-07-09 against a marked disposable Tahoe `26.5.1` arm64 account after a real loginwindow Aqua session created `gui/502`. The run used package SHA-256 `cfff83af6fd1dfc59ea1ed4928befe53929462eabd096e5c54d6171379be7ccc`, official channel policy `official-openai-github-release`, stock Codex release `0.143.0`, release URL `https://github.com/openai/codex/releases/download/rust-v0.143.0/codex-aarch64-apple-darwin.tar.gz`, and archive SHA-256 `7df2384f037519dff7dbf4252e60913a5c1c7fdb66c1467c9125b2b2d3594a86`. All five aggregate steps reported `replacement-ready` with `blocked_step=None`: remote acquisition, auth onboarding, auth persistence, update-agent, and live. Evidence recorded host reference `codex-cli 0.142.5`, `host_stock_codex_uploaded_any=False`, clean-target selected `codex-cli 0.143.0` for auth onboarding, auth persistence, update-agent, and live, auth-persistence thread `019f46e8-6186-70c1-8098-c73c17834725`, live thread `019f46e9-c01d-7e53-82aa-e74401a04fb3`, update-agent `launch_domain=gui/502`, `launchctl_bootstrap=loaded`, `launchctl_kickstart=completed`, `launchctl_bootout=unloaded`, and `scheduled_action=up-to-date`. A final `stock-codex-compat-pkg-nontart-clean-machine-preflight` rerun reported the target clean again: no package receipt, payload, launcher, manifest, adapter root, stock cache, or LaunchAgent residue. This adds direct clean-Mac release evidence for the signed package/runtime path without Tart. It still requires an operator-prepared disposable account with key SSH, `uvx`, noninteractive `sudo -n`, the clean-user marker, and a real `gui/<uid>` Aqua session for the LaunchAgent step; SSH-only background `user/<uid>` sessions fail even a minimal LaunchAgent bootstrap with `Bootstrap failed: 5: Input/output error`. It does not automate browser/device login, prove Keychain-managed auth, decide LaunchDaemon policy, replace archived wrapper JSON evidence, or broaden the claim to non-disposable user profiles. |
| Signed/notarized stock Codex compatibility pkg direct clean-Mac install/provisioning | `replacement-ready` for release-evidence-gated non-Tart package install, official stock Codex provisioning, and rollback cleanup | `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-nontart-clean-machine-install --codex-path /Users/joshuakaunert/.local/omnigent/codex-stock/0.142.5/codex --pkg-path /Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.pkg --release-evidence-path /Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.direct-clean-mac.release-evidence.json --clean-vm-ssh-target omnigent-clean@10.0.0.10 --clean-vm-ssh-identity /Users/joshuakaunert/.ssh/mba_github_ssh_key` passed on 2026-07-09. The proof first reran the direct clean-machine preflight and release evidence verifier, then installed package SHA-256 `cfff83af6fd1dfc59ea1ed4928befe53929462eabd096e5c54d6171379be7ccc` on the operator-supplied disposable Tahoe `26.5.1` arm64 target without Tart, bootstrapped the user runtime, acquired stock Codex `codex-cli 0.143.0` from `https://github.com/openai/codex/releases/download/rust-v0.143.0/codex-aarch64-apple-darwin.tar.gz`, installed the default `fetch_apple_docs` adapter package, selected `~/.local/bin/omnigent-stock-codex-compat`, and reported `host_stock_codex_uploaded=False`, `tart_started=False`, and `tart_stopped=False`. A follow-up `stock-codex-compat-pkg-nontart-clean-machine-preflight` rerun reported the target clean again: no package receipt, payload, launcher, manifest, adapter root, stock cache, or LaunchAgent residue. Unit coverage in `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_nontart_clean_machine_install_blocks_before_install` and `test_stock_codex_compat_pkg_nontart_clean_machine_install_runs_direct_remote_acquisition` covers preflight-before-install ordering, fail-closed release evidence blocking, direct-SSH-only dispatch, no Tart use, and proof output. This closes non-Tart package install/provisioning execution on a disposable clean machine; it does not automate browser/device login, prove persistent LaunchAgent policy, or broaden the claim to non-disposable user profiles. |
| Signed/notarized stock Codex compatibility pkg direct clean-Mac auth onboarding | `replacement-ready` for non-Tart installed-launcher guided login setup without credential upload or browser-login execution | `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-nontart-clean-machine-auth-onboarding --codex-path /Users/joshuakaunert/.local/omnigent/codex-stock/0.142.5/codex --pkg-path /Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.pkg --release-evidence-path /Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.direct-clean-mac.release-evidence.json --clean-vm-ssh-target omnigent-clean@10.0.0.10 --clean-vm-ssh-identity /Users/joshuakaunert/.ssh/mba_github_ssh_key` passed on 2026-07-09. The proof first reran the direct clean-machine preflight and release evidence verifier, then installed package SHA-256 `cfff83af6fd1dfc59ea1ed4928befe53929462eabd096e5c54d6171379be7ccc` on the operator-supplied disposable Tahoe `26.5.1` arm64 target without Tart, bootstrapped the user runtime, acquired stock Codex `codex-cli 0.143.0` from the official OpenAI GitHub release channel, selected `/Users/omnigent-clean/.local/bin/omnigent-stock-codex-compat`, classified `/Users/omnigent-clean/.omnigent-stock-codex-compat-clean-vm-remote-acquisition-proof/auth-onboarding-codex-home/auth.json` as `needs-auth`, and emitted `CODEX_HOME=/Users/omnigent-clean/.omnigent-stock-codex-compat-clean-vm-remote-acquisition-proof/auth-onboarding-codex-home /Users/omnigent-clean/.local/bin/omnigent-stock-codex-compat login`. Evidence recorded `host_stock_codex_uploaded=False`, `command_executed=False`, `auth_uploaded=False`, `tart_started=False`, `tart_stopped=False`, release evidence status `verified`, clean-target selected `codex-cli 0.143.0`, and a follow-up `stock-codex-compat-pkg-nontart-clean-machine-preflight` rerun reported no package receipt, payload, launcher, manifest, adapter root, stock cache, or LaunchAgent residue. Unit coverage in `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_nontart_clean_machine_auth_onboarding_blocks_before_install`, `test_stock_codex_compat_pkg_nontart_clean_machine_auth_onboarding_runs_direct_gate`, and `test_nontart_clean_machine_auth_onboarding_blocks_stale_selected_version` covers preflight-before-auth ordering, release-evidence gating, direct-SSH-only dispatch, no Tart use, no auth upload, no login execution, exact installed-launcher login command parsing, and fail-closed stale selected-version rejection. This closes non-Tart guided auth setup on a disposable clean machine; it does not automate browser/device login, validate token freshness, prove token persistence after user login, prove Keychain-managed auth, run a live model turn, schedule launchd updates, or broaden the claim to non-disposable user profiles. |
| Signed/notarized stock Codex compatibility pkg direct clean-Mac operator-assisted auth login | `replacement-ready` after operator-completed wrapper login without host credential upload | `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-nontart-clean-machine-auth-login --codex-path /Users/joshuakaunert/.local/omnigent/codex-stock/0.142.5/codex --pkg-path /Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.pkg --release-evidence-path /Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.direct-clean-mac.release-evidence.json --clean-vm-ssh-target omnigent-clean@10.0.0.10 --clean-vm-ssh-identity /Users/joshuakaunert/.ssh/mba_github_ssh_key --clean-vm-remote-codex-home /Users/omnigent-clean/.codex-omnigent-wrapper-auth --live-proof-timeout 600` passed as the no-host-auth-upload preparation gate for the existing-auth verifier. It first reruns release-evidence verification and direct clean-machine preflight, refuses Tart and host auth upload, requires a clean remote `CODEX_HOME` under the remote user's home, blocks if that home already contains `auth.json`, installs/provisions the signed package from the official channel, executes `CODEX_HOME=<remote-auth-home> ~/.local/bin/omnigent-stock-codex-compat login`, waits for operator-completed browser/device auth within `--live-proof-timeout`, verifies the resulting `auth.json` is available at the requested remote path with mode `0o600`, then removes Omnigent package/user state while preserving the remote auth home. Unit coverage in `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_nontart_clean_machine_auth_login_blocks_before_install`, `test_stock_codex_compat_pkg_nontart_clean_machine_auth_login_requires_absolute_home`, `test_stock_codex_compat_pkg_nontart_clean_machine_auth_login_runs_direct_gate`, `test_nontart_clean_machine_auth_login_blocks_if_command_not_executed`, `test_stock_codex_compat_pkg_clean_vm_auth_login_uses_remote_home_without_auth_upload`, and `test_clean_vm_auth_login_output_evidence_requires_login_execution` covers preflight-before-login ordering, absolute remote auth-home enforcement, direct-SSH-only dispatch, no Tart use, no auth upload, command execution, post-login auth availability, selected-version checks, and fail-closed evidence parsing. This gate intentionally does not automate browser/device login; it productizes the user-completed login transaction so the following existing-auth verifier can run a live model turn without copying host credentials. |
| Signed/notarized stock Codex compatibility pkg direct clean-Mac existing-auth verifier | `replacement-ready` with wrapper-created remote `CODEX_HOME` and no credential upload | `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-nontart-clean-machine-auth-persistence --codex-path /Users/joshuakaunert/.local/omnigent/codex-stock/0.142.5/codex --pkg-path /Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.pkg --release-evidence-path /Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.direct-clean-mac.release-evidence.json --clean-vm-ssh-target omnigent-clean@10.0.0.10 --clean-vm-ssh-identity /Users/joshuakaunert/.ssh/mba_github_ssh_key --clean-vm-remote-codex-home /Users/omnigent-clean/.codex-omnigent-wrapper-auth` passed as the no-host-auth-upload verifier. It reruns release-evidence verification and direct clean-machine preflight, refuses Tart and host auth upload, requires the remote `CODEX_HOME` to already contain `auth.json`, installs/provisions the signed package, runs a live model turn through the installed compatibility launcher with `CODEX_HOME=<remote-auth-home>`, verifies the auth file remains available after the live turn, then cleans Omnigent package/user state while leaving the operator-provided auth home in place. Unit coverage in `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_nontart_clean_machine_auth_persistence_blocks_before_install`, `test_stock_codex_compat_pkg_nontart_clean_machine_auth_persistence_requires_absolute_home`, `test_stock_codex_compat_pkg_nontart_clean_machine_auth_persistence_runs_direct_gate`, and `test_nontart_clean_machine_auth_persistence_blocks_uploaded_auth` covers preflight-before-install ordering, absolute remote auth-home enforcement, direct-SSH-only dispatch, no Tart use, no auth upload, live installed-launcher evidence parsing, and fail-closed rejection if uploaded auth is reported. The disposable target now has the wrapper-created remote auth home, and the live verifier passed while preserving it. |
...
| Immutable stock-Codex compatibility release promotion | `replacement-ready` for the first commit-bound direct clean-Mac package/evidence/manifest transaction | `scripts/promote_stock_codex_compat_release.py` promoted clean pushed source commit `985c7295f347caafd238cfc4c036bff3615e7310` into immutable directory `omnigent-stock-codex-compat-0.3.0.dev0-985c7295f347`. The live command used explicit signer `Developer ID Installer: Joshua Kaunert (HSRQC9N69B)` and notary profile `OmnigentExperiment`, produced package SHA-256 `adae5f4359fd66f62354de6a57fc8284a0e46771d5ba850b21729ab93e296302`, source-bundle SHA-256 `b186fb1d344301258e30cf2f00e6e12129f363ff06b8a1fb1959bf124ce5dac9`, accepted notary submission `6563c3a9-57db-475b-a329-959a7427376d`, release-evidence SHA-256 `9b08ae3e783e24687a765af36db68d29375ab8342fdee251d8526f286817d36a`, and promotion-manifest SHA-256 `0d006703bd75fb67c26d7f3eed70c263efdc45cbc1993da1fc54c7256b3ed26d`. All five direct-target steps selected official stock Codex `0.144.1` and reported `replacement-ready`; no host stock Codex or auth was uploaded. A standalone `--verify-only` rerun passed, and a final direct-target preflight proved no receipt, payload, launcher, manifest, adapter root, stock cache, or LaunchAgent residue while revalidating signature, staple, and Gatekeeper acceptance. Focused adversarial coverage includes partial-output cleanup, local/remote provenance rejection, tracked-tool binding, exact signer checks, symlink/tamper rejection, embedded metadata drift, and relocated-directory verification. That historical development-version promotion did not itself publish artifacts or finalize a stable release; the immutable public `0.1.3` row below records those later steps. Rollout/feed policy remains separate. |
| Public stock-Codex compatibility release channel | `replacement-ready` for stable versioning, immutable GitHub publication with signed release attestations, unauthenticated public asset verification, and clean-target URL installation | Release [`stock-codex-compat-v0.1.3`](https://github.com/jkaunert/omnigent/releases/tag/stock-codex-compat-v0.1.3) was published on 2026-07-10 from clean pushed source commit `22a1b355d8acd6d2411c03ee5ee8cc682ef3f8b5`. `packaging/stock-codex-compat/VERSION` owns installer version `0.1.3` independently from Omnigent's lockstep PyPI version. Publication schema v2 requires `releaseImmutable=true`; the publisher refuses to start unless repository release immutability is enabled and rejects a public API result without `immutable=true`. The immutable release contains signed/notarized package SHA-256 `6cadc5d884ae8225544ea26e26c8b4f7f713d72e2869eeccbeeca144d9917cc5`, release-evidence SHA-256 `46f56b00730881022b812d113f048898ba05c380ed8722e2b46fb9d0d2397248`, promotion-manifest SHA-256 `eecbc19c11639ebec6f3f570a6784150292542ba8b9519473b91f7eda494a5e9`, `SHA256SUMS` SHA-256 `dd29c5ad401f44dcf584fbc60c68c51e37464fe895864abf5c8b43bffaec86be`, and publication-record SHA-256 `707d1e8f0a6da94b9ef05eec7b28ed58d209cce04e08eec1b8e95635a5c64f81`; the release body anchors the publication-record digest. Accepted notarization submission `31eb8423-43f3-4b60-83b5-acea88f01a78` and the five-step release evidence select official stock Codex `0.144.1`, with every step `replacement-ready` and no host stock Codex or auth upload. GitHub reports `isImmutable=true`; `gh release verify` validated the signed attestation over the protected tag plus all five asset digests, and `gh release verify-asset` validated the local package bytes. The publisher also verified the draft asset set/hashes, re-read the public API, and downloaded every asset without credentials; a separate schema-v2 `--verify-only` run passed. `scripts/prove_stock_codex_compat_published_release.py` then sent only a shell program to the marked disposable Tahoe `26.5.1` arm64 Mac, downloaded the package from the immutable public GitHub URL, verified SHA-256/signature/staple/Gatekeeper, installed and checked receipt/payload version `0.1.3`, removed the payload and receipt, and confirmed cleanup with `published_release_package_uploaded=False`, `published_release_auth_uploaded=False`, and `published_release_cleanup=complete`. Adversarial coverage includes disabled-immutability rejection, mutable-public-release rejection, GitHub release and package attestation commands, tag namespace, draft temporary-URL normalization/token binding, release-body digest anchoring, asset-set/hash mismatch, failed-draft cleanup, no-package-upload command shape, cleanup-marker enforcement, and uniquely correlated recovery from a post-upload `notarytool` crash. The earlier `0.1.2` release was a mutable publication rehearsal; `0.1.3` is the production channel target. This closes the trusted-publication, release-immutability, signed-attestation, and clean-machine public-acquisition production gap; rollout/feed policy remains a separate product decision. |
...
The wrapper-created remote auth home
`/Users/omnigent-clean/.codex-omnigent-wrapper-auth` now exists with mode-`0600`
`auth.json`; the separate no-upload auth-persistence proof passed, and the
canonical direct five-step release artifact now records
`authUploaded=false` for onboarding, persistence, and live while selecting
stock Codex `0.144.1`. The current canonical artifact and criteria in the
release-candidate section below supersede those earlier blocked/uploaded-auth
descriptions.
...
the installed runtime, installs and selects `~/.local/bin/omnigent-stock-codex-compat`
from that installed runtime, then runs the packaged runtime's auth classifier
with `PYTHONPATH=<installed-runtime>`. It verifies the real stock auth source is
...
scope, classifier output does not leak synthetic credential material, and the
guided login command targets the compatibility launcher rather than the raw
stock Codex binary. This closes clean auth classification and launcher-owned
...
bootstrap-backed credential rerun found `pkgbuild`, `pkgutil`, `xcrun`,
`spctl`, `notarytool`, and `stapler`; autodiscovered `Developer ID Installer:
Joshua Kaunert (HSRQC9N69B)`; used notarytool keychain profile
`OmnigentExperiment`; included the packaged first-run bootstrap payloads;
submitted and stapled final package SHA-256
`7ec3bec64157f1bed9f56ad1e457ef556912ca3ddd1ac95db5b534066bdf2f3d`; received
notary submission `7256d678-f5cc-4676-ba35-4d3db4b28820` with status
...
real `/Library` package install plus the packaged
`runtime/scripts/bootstrap_stock_codex_compat.sh` first-run bootstrapper. That
bootstrapper stages the immutable machine runtime into the VM user's writable
`~/.local/omnigent/stock-codex-compat/runtime`, runs the Python second stage
through `uvx --from <staged-runtime>`, provisions stock Codex, installs the
adapter package, installs the launcher, and runs doctor before the proof
continues with auth classification, rollback, payload cleanup, and receipt
cleanup. It requires the same disposable home marker, key SSH, `uvx`, and
noninteractive `sudo -n` inside the VM, so it blocks cleanly for ordinary
desktop VMs or unprepared CI images. On 2026-07-07, Tart `2.32.1` ran the live
gate against `omnigent-clean`; the signed package installed into the VM's real
`/Library`, the packaged bootstrapper reported `action=bootstrapped`, clean auth
classified as `needs-auth`, and package/user state cleaned up.

2026-07-07 clean-VM remote acquisition note:
`stock-codex-compat-pkg-clean-vm-remote-acquisition` closes the next
machine-portability gap by copying only the signed package, a URL-backed
official OpenAI GitHub release channel manifest, and the proof script into the
VM. The packaged bootstrapper now accepts `--channel-policy` and forwards it to
the bundled provisioner. The live gate installed the branch-built
signed/notarized package into the VM's real `/Library`, downloaded and verified
the stable `codex-aarch64-apple-darwin.tar.gz` release archive inside the VM,
proved `host_stock_codex_uploaded=False`, bootstrapped the compatibility
launcher, classified clean auth as `needs-auth`, then removed user state,
package payload, receipt, and remote workdir state.

2026-07-08 clean-VM update-agent launchd note:
`stock-codex-compat-pkg-clean-vm-update-agent` closes the scheduled-updater
enablement gap by extending the same URL-backed clean-VM install path with a
user LaunchAgent load and forced scheduled run. The proof first exposed a real
launchd environment bug: the generated plist invoked absolute `uvx`, but the
scheduled updater process did not receive `--uvx-path`, so launchd's sparse
PATH made the updater fail to rediscover `uvx`. The updater now preserves the
absolute `uvx` path in scheduled `ProgramArguments`. The fixed signed/notarized
package loaded `/Users/admin/Library/LaunchAgents/ai.omnigent.stock-codex-compat.update.plist`
into `gui/501`, ran `launchctl kickstart -k`, parsed scheduled updater JSON
with `scheduled_action=up-to-date`, unloaded with `launchctl bootout`, selected
the in-VM stock Codex path, and proved `host_cache_referenced=False`.

2026-07-08 clean-VM auth-onboarding note:
`stock-codex-compat-pkg-clean-vm-auth-onboarding` closes the guided clean-machine
auth setup boundary. It extends the same URL-backed clean-VM install path, but
does not upload `auth.json` and does not execute browser/device login. The proof
installs the signed/notarized package into the disposable VM's real `/Library`,
acquires stock Codex from the official OpenAI GitHub release channel inside the
VM, verifies the installed launcher selected on `PATH`, classifies a proof-scoped
clean `CODEX_HOME` as `needs-auth`, and emits the operator command
`CODEX_HOME=/Users/admin/.omnigent-stock-codex-compat-clean-vm-remote-acquisition-proof/auth-onboarding-codex-home /Users/admin/.local/bin/omnigent-stock-codex-compat login`.
Evidence recorded `host_stock_codex_uploaded=False`, `auth_uploaded=False`,
`command_executed=False`, and `tart_stopped=True`. This still leaves automated
browser/device login, Keychain-managed credential UX, and account refresh
semantics as separate product gates.

2026-07-08 clean-VM auth-persistence note:
`stock-codex-compat-pkg-clean-vm-auth-persistence` closes the post-login
file-backed credential boundary for the installed launcher. It uses the same
guided `CODEX_HOME` from auth onboarding, confirms that it starts as
`needs-auth`, copies a proof-scoped stock `auth.json` there as the stand-in for
a user-completed login, confirms the installed runtime now classifies it as
available, and runs a real installed-launcher model turn with deterministic
route evidence before `STOCK_CODEX_COMPAT_AUTH_PERSISTENCE_OK`. Evidence
recorded `auth_uploaded=True`, `host_stock_codex_uploaded=False`,
`login_command_executed=False`, `browser_login_automated=False`,
`auth_persisted_after_live=True`, live thread
`019f42b9-db2c-71f1-8c73-95d486b6c64e`, and `tart_stopped=True`. This still
...
2026-07-09 direct clean-Mac release aggregate note:
The same `stock-codex-compat-pkg-clean-vm-release` aggregate also passed
against the operator-supplied non-Tart target `omnigent-clean@10.0.0.10` with
the signed/notarized package artifact at
`/Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.pkg`.
The target was a marked disposable Tahoe `26.5.1` arm64 account with key SSH,
user-local `uvx`, noninteractive `sudo -n`, and a loginwindow-created Aqua
session exposing `gui/502`. The aggregate reported all five steps
`replacement-ready`, `blocked_step=None`, `host_stock_codex_uploaded_any=False`,
official channel policy `official-openai-github-release`, stock Codex release
`0.143.0`, host reference `codex-cli 0.142.5`, and clean-target selected
`codex-cli 0.143.0` for auth onboarding, auth persistence, update-agent, and
live. Evidence recorded auth-persistence thread
`019f46e8-6186-70c1-8098-c73c17834725` and live thread
`019f46e9-c01d-7e53-82aa-e74401a04fb3`. The update-agent step
loaded `/Users/omnigent-clean/Library/LaunchAgents/ai.omnigent.stock-codex-compat.update.plist`
into `gui/502`, completed `launchctl kickstart -k`, unloaded it, and reported
`scheduled_action=up-to-date` with `selected_codex_version=codex-cli 0.143.0`
and `host_cache_referenced=False`.

The failed attempt before GUI login is part of the operator contract. SSH-only
account setup created `user/502` but not `gui/502`; both the real updater plist
and a minimal test LaunchAgent failed to bootstrap there with
`Bootstrap failed: 5: Input/output error`. The direct clean-Mac update-agent
gate is therefore a GUI-session proof, not a pure headless SSH proof. Operators
must keep the disposable account logged into an Aqua session before running the
aggregate when the scheduled updater/LaunchAgent step is in scope. After the
aggregate, a fresh `stock-codex-compat-pkg-nontart-clean-machine-preflight`
reported the target clean again: no package receipt, payload, launcher,
manifest, adapter root, stock cache, or LaunchAgent residue.

2026-07-09 direct clean-Mac auth-onboarding note:
`stock-codex-compat-pkg-nontart-clean-machine-auth-onboarding` closes the
standalone guided-login boundary for an operator-supplied non-Tart target. The
gate composes the release-evidence verifier and direct clean-machine preflight
with the installed-launcher auth-onboarding path. It passed against
`omnigent-clean@10.0.0.10` with package SHA-256
`cfff83af6fd1dfc59ea1ed4928befe53929462eabd096e5c54d6171379be7ccc`,
official channel policy `official-openai-github-release`, clean-target selected
stock Codex `codex-cli 0.143.0`, `host_stock_codex_uploaded=False`,
`auth_uploaded=False`, `command_executed=False`, and no Tart start/stop. The
proof classified the proof-scoped clean `CODEX_HOME` as `needs-auth` and emitted
the user-run login command through the installed compatibility launcher:
`CODEX_HOME=/Users/omnigent-clean/.omnigent-stock-codex-compat-clean-vm-remote-acquisition-proof/auth-onboarding-codex-home /Users/omnigent-clean/.local/bin/omnigent-stock-codex-compat login`.
A follow-up direct clean-machine preflight reported the target clean again.

2026-07-09 direct clean-Mac operator-assisted auth-login note:
`stock-codex-compat-pkg-nontart-clean-machine-auth-login` adds the missing
no-host-auth-upload preparation gate for the product UX where the user completes
login on the target. The gate accepts
`--clean-vm-remote-codex-home <remote-absolute-path>`, requires that path to be
clean and inside the remote user's home, installs and provisions the signed
package from the official channel, executes the installed compatibility launcher
`login` command with `CODEX_HOME` set to that remote auth home, waits for
operator-completed browser/device auth within `--live-proof-timeout`, verifies
the resulting `auth.json` is available, and removes Omnigent/package state while
leaving the remote auth home intact. It does not automate browser/device login
or upload host credentials.

2026-07-09 direct clean-Mac existing-auth verifier note:
`stock-codex-compat-pkg-nontart-clean-machine-auth-persistence` adds the
no-host-auth-upload verifier for the product UX choice where the user completes
login on the target instead of the proof copying credentials. The gate accepts
the same `--clean-vm-remote-codex-home <remote-absolute-path>`, requires that
path to already contain `auth.json`, installs and provisions the signed package
from the official channel, runs a live model turn through the installed
compatibility launcher with that remote `CODEX_HOME`, verifies the auth remains
available after the turn, then removes Omnigent/package state while leaving the
operator-provided auth home intact. On `omnigent-clean@10.0.0.10`, the
operator-assisted wrapper login created
`/Users/omnigent-clean/.codex-omnigent-wrapper-auth/auth.json`; the verifier and
the canonical release aggregate both passed against that file without uploading
credentials and preserved it after cleanup.
...
target VM, clones a cached source image, randomizes the clone MAC, waits for both
IP and Tart guest-agent readiness, injects the supplied SSH public key, marks the
guest home disposable, optionally installs user-local `uvx` without editing shell
startup files, verifies `sudo -n`, verifies key SSH independently, and stops the
clone. The live run prepared `omnigent-clean-bootstrap-proof` from
`ghcr.io/cirruslabs/macos-tahoe-base:latest`, then the downstream
`stock-codex-compat-pkg-clean-vm-update-agent` gate passed on that freshly
prepared clone with package SHA-256
`d0185a22380036b97703144f91bbcb701806c3638ebcbe98b2aa38df43baf581`,
`launch_domain=gui/501`, `scheduled_action=up-to-date`,
`host_stock_codex_uploaded=False`, and `host_cache_referenced=False`.
...
and live installed-launcher gates, plus standalone direct clean-Mac
install/provisioning, guided auth-onboarding, operator-assisted auth-login, and
no-host-auth-upload existing-auth verifier gates. Direct clean-Mac targets can
use the same aggregate through the release wrapper when the operator prepares a
disposable SSH account with a real `gui/<uid>` Aqua session for the
LaunchAgent step. The package release-candidate gate is the
...
target mode, host-stock-upload result, clean-target selected stock-Codex
versions, and live/auth thread details.
...
automated browser/device login UX beyond the operator-assisted gate, token
freshness validation, Keychain-managed credential UX, and broader UI/device
...
Pkg-installed runtime scheduled updater:

```bash
uvx --from . python scripts/prove_stock_codex_replacement.py \
  --proof stock-codex-compat-pkg-update-agent \
  --codex-path ~/.local/omnigent/codex-stock/0.142.5/codex
```

The `stock-codex-compat-pkg-update-agent` proof fails closed if `--apple-bundle`
or `--allow-fork-codex` are supplied. It builds and expands the unsigned `.pkg`,
stages the runtime under a temporary installed-root shape, verifies the packaged
`runtime/scripts/update_stock_codex_compat.py` payload and manifest contract,
writes a user LaunchAgent plist without loading it, then runs the updater once.
The generated plist delegates to `uvx --from <installed-runtime> python
<installed-runtime>/scripts/update_stock_codex_compat.py`, passes
`--uvx-path <absolute-uvx>` for launchd execution, follows the current launcher
manifest instead of baking `--current-codex`, and omits proof-only
`--write-launch-agent`/`--run-now` flags. The direct updater run stages the
stable official release target, promotes the clean user's launcher pointer and
env contract, writes rollback metadata, and a second run reports `up-to-date`
without mutation or another remote-download flag.
It does not load launchd state, decide final LaunchAgent versus LaunchDaemon
policy, adopt pre-release channels, verify an independent archive signature,
perform a real `/Library` install, or prove Gatekeeper behavior.

Clean-VM raw Tart bootstrap gate:

```bash
uvx --from . python scripts/prove_stock_codex_replacement.py \
  --proof stock-codex-compat-pkg-clean-vm-bootstrap \
  --clean-vm-source-tart-name ghcr.io/cirruslabs/macos-tahoe-base:latest \
...
`stock-codex-compat-pkg-clean-vm-bootstrap` prepares the disposable Tart VM
contract that the package-consuming clean-VM gates require. It clones from a
cached source image into a new target name, refuses to overwrite existing VMs,
starts the clone headless, waits for Tart guest-agent readiness, injects the SSH
public key derived from the supplied identity, marks the home with
`.omnigent-stock-codex-compat-clean-user-ok`, installs `uvx` only when
`--clean-vm-bootstrap-install-uv` is supplied, verifies `sudo -n`, verifies key
SSH independently, and stops the clone. The follow-on update-agent gate passed
against the newly prepared target `omnigent-clean-bootstrap-proof`, proving the
bootstrap output can consume the signed/notarized package path. This does not
prove non-Tart clean-machine provisioning or automated browser auth.

Clean-VM scheduled updater launchd gate:

```bash
uvx --from . python scripts/prove_stock_codex_replacement.py \
  --proof stock-codex-compat-pkg-clean-vm-update-agent \
  --codex-path ~/.local/omnigent/codex-stock/0.142.5/codex \
  --pkg-path /private/tmp/omnigent-stock-codex-compat-update-agent-pkg-fixed.HhDwoi/omnigent-stock-codex-compat.pkg \
  --clean-vm-tart-name omnigent-clean \
  --clean-vm-ssh-user admin \
  --clean-vm-ssh-identity ~/.ssh/mba_github_ssh_key \
  --clean-vm-start-tart
```

`stock-codex-compat-pkg-clean-vm-update-agent` consumes a signed/notarized
package and the same prepared disposable Tart VM contract as the other clean-VM
gates. It installs into the VM's real `/Library`, bootstraps the staged user
runtime from the official URL-backed channel, writes the user LaunchAgent,
validates that scheduled `ProgramArguments` preserve the absolute `uvx` path,
loads it with `launchctl bootstrap`, forces one run with `launchctl kickstart`,
...
`OMNIGENT_STOCK_CODEX_COMPAT_RELEASE_SSH_PORT`. Direct-SSH release runs also
require `OMNIGENT_STOCK_CODEX_COMPAT_RELEASE_REMOTE_CODEX_HOME` or the matching
CLI option. The optional evidence path can
...
direct SSH mode. The evidence must also show that auth-onboarding,
auth-persistence, update-agent, and live steps selected `codex-cli <caskVersion>`
from the package-managed official channel. The host reference Codex path printed
by the aggregate is allowed to lag the package channel, but it is not allowed to
be uploaded to the clean target or stand in for clean-target selected-version
evidence. Direct-SSH evidence additionally requires an absolute
`remote-existing-codex-home:<path>` auth source, `<path>/auth.json` availability,
and `authUploaded=false` for auth-onboarding, auth-persistence, and live. This
prevents a host credential copy from satisfying the production direct-target
gate.
...
First live promotion evidence on 2026-07-09:

- Promoted source commit:
  `985c7295f347caafd238cfc4c036bff3615e7310`.
- Release directory:
  `/Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/releases/omnigent-stock-codex-compat-0.3.0.dev0-985c7295f347`.
- Package SHA-256:
  `adae5f4359fd66f62354de6a57fc8284a0e46771d5ba850b21729ab93e296302`.
- Release-evidence SHA-256:
  `9b08ae3e783e24687a765af36db68d29375ab8342fdee251d8526f286817d36a`.
- Promotion-manifest SHA-256:
  `0d006703bd75fb67c26d7f3eed70c263efdc45cbc1993da1fc54c7256b3ed26d`.
- Source-bundle SHA-256:
  `b186fb1d344301258e30cf2f00e6e12129f363ff06b8a1fb1959bf124ce5dac9`.
- Accepted notarization submission:
  `6563c3a9-57db-475b-a329-959a7427376d`.
- Official stock-Codex selection: `0.144.1`, archive SHA-256
  `88e72ac8bd30815f7d18e62dac333dc20ce3ad1cba94be1649a1977dd9bfdbb8`.
- Auth-persistence thread: `019f49db-8328-7122-8bd8-35e14466b4e0`.
- Live thread: `019f49dc-c51d-7ed0-9efe-3783e32bc77b`.

All five release steps were `replacement-ready`, both release exit codes were
zero, `releaseCriteriaFailures=[]`, `hostStockCodexUploadedAny=false`, and
direct-target auth remained in the existing remote `CODEX_HOME` with
`authUploaded=false`. A standalone `--verify-only` run passed against the
completed directory. The final non-Tart preflight revalidated the package and
evidence and found no package receipt, payload, launcher, launcher manifest,
...
uv run python scripts/publish_stock_codex_compat_release.py \
  --promotion-dir /absolute/path/to/promoted-0.1.3 \
  --output-dir /absolute/path/to/publication-0.1.3 \
  --repository jkaunert/omnigent \
  --tag stock-codex-compat-v0.1.3

uv run python scripts/publish_stock_codex_compat_release.py \
  --verify-only /absolute/path/to/publication-0.1.3/publication-record.json

uv run python scripts/prove_stock_codex_compat_published_release.py \
  --publication-record /absolute/path/to/publication-0.1.3/publication-record.json \
...
not from the development host. The real `0.1.3` run passed immutable
publication, GitHub release and package attestation verification, independent
public verification, direct clean-Mac installation, receipt and payload
inspection, rollback cleanup, and final absence checks, so this gate is
`replacement-ready`.
...
Current canonical direct-target release-candidate evidence:
`/Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.direct-clean-mac.release-evidence.json`
recorded `status=replacement-ready`, evidence SHA-256
`4a8bd8f1774456fb22c0314170ceb946cff7f3db41df6ceb2f4aa0b06cc2dceb`,
package SHA-256
`2bff223bc5ec67282a8b631bd708d5f354db06a0ab6626867f5ff84eb341e237`,
stock Codex reference `codex-cli 0.142.5`, official channel version `0.144.1`
with archive SHA-256
`88e72ac8bd30815f7d18e62dac333dc20ce3ad1cba94be1649a1977dd9bfdbb8`,
all five step statuses as `replacement-ready`, `blockedStep=None`, zero Tart
starts/stops, and `hostStockCodexUploadedAny=False`. Its auth source is the
wrapper-created remote home
`/Users/omnigent-clean/.codex-omnigent-wrapper-auth`; auth-onboarding,
auth-persistence, and live all recorded `authUploaded=false`. Auth-persistence
thread `019f4968-df5d-78c0-92e2-17b2823ed94f` and live thread
`019f496a-4a63-79b0-a1c0-b6bd1545673a` both selected `codex-cli 0.144.1` through
the installed compatibility launcher.
...
Tart VM name. It requires an absolute remote `CODEX_HOME` that already contains
a nonempty mode-`0600` `auth.json`; the preflight returns only availability and
mode metadata, and the installed target runtime uses the file in place without
uploading or returning credential contents.
It expects the operator to prepare and mark the disposable account
outside the proof: SSH key access, `~/.omnigent-stock-codex-compat-clean-user-ok`,
user-local `uvx`, noninteractive `sudo -n`, and no existing Omnigent
stock-Codex-compat state. If the update-agent step is in scope, the account
...
official-channel, step-status, selected-version, no-host-stock-upload, and
live/auth thread criteria. It additionally rejects any direct-target artifact
whose auth source is not that remote home or whose auth-onboarding,
auth-persistence, or live step reports `authUploaded` other than `false`, then
writes the machine-readable evidence JSON beside the `.pkg`. Online and offline
validation also require the recorded SSH target and remote auth home to match the
corresponding command arguments exactly.
...
The direct clean-machine install/provisioning proof composes that preflight with
the official remote-acquisition package path. It intentionally does not cover
auth login, live model execution, or update scheduling; those remain separate
non-Tart gates so installation, account state, and launchd behavior stay
diagnosable.

Direct clean-machine auth-onboarding proof composes the same preflight with the
installed-launcher guided login boundary:

```bash
uvx --from . python scripts/prove_stock_codex_replacement.py \
  --proof stock-codex-compat-pkg-nontart-clean-machine-auth-onboarding \
  --codex-path ~/.local/omnigent/codex-stock/0.142.5/codex \
  --pkg-path /Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.pkg \
  --release-evidence-path /Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.direct-clean-mac.release-evidence.json \
  --clean-vm-ssh-target omnigent-clean@10.0.0.10 \
  --clean-vm-ssh-identity ~/.ssh/mba_github_ssh_key
```

This proof intentionally refuses Tart arguments, refuses host stock-Codex upload,
requires the direct SSH target to include the remote user, verifies the release
evidence before touching the target, confirms the installed launcher selects the
release-evidence cask version, classifies the clean proof-scoped `CODEX_HOME` as
`needs-auth`, and emits the operator-run `login` command without executing it or
uploading credentials. It does not prove token persistence, browser/device auth
automation, live model execution, or launchd update behavior.

Direct clean-machine operator-assisted auth login composes the same preflight
with a persistent remote auth home that the operator will authenticate on the
target:

```bash
uvx --from . python scripts/prove_stock_codex_replacement.py \
  --proof stock-codex-compat-pkg-nontart-clean-machine-auth-login \
  --codex-path ~/.local/omnigent/codex-stock/0.142.5/codex \
  --pkg-path /Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.pkg \
  --release-evidence-path /Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.direct-clean-mac.release-evidence.json \
  --clean-vm-ssh-target omnigent-clean@10.0.0.10 \
...
This proof refuses Tart arguments and host credential upload. The remote
`CODEX_HOME` must be a clean path under the remote user's home; if `auth.json`
already exists there, run the existing-auth verifier instead. During the proof,
the operator must complete the stock Codex login flow opened by the installed
compatibility launcher before `--live-proof-timeout` expires. On success, the
proof preserves that remote auth home and removes proof-owned Omnigent/package
state.

Direct clean-machine existing-auth verification composes the same preflight with
a remote auth home that the operator already authenticated on the target:

```bash
uvx --from . python scripts/prove_stock_codex_replacement.py \
  --proof stock-codex-compat-pkg-nontart-clean-machine-auth-persistence \
  --codex-path ~/.local/omnigent/codex-stock/0.142.5/codex \
  --pkg-path /Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.pkg \
  --release-evidence-path /Users/joshuakaunert/Developer/HarnessEngineering/omnigent-proof-artifacts/omnigent-stock-codex-compat-github-latest.direct-clean-mac.release-evidence.json \
  --clean-vm-ssh-target omnigent-clean@10.0.0.10 \
...
This proof refuses Tart arguments and host credential upload. The remote
`CODEX_HOME` must already contain `auth.json`; the proof uses it for a live
installed-launcher model turn, proves the auth remains available after the turn,
and then removes proof-owned Omnigent/package state while preserving that remote
auth home. On a target without that preexisting login, this gate blocks before
claiming replacement readiness.
...
installed runtime, installs and selects the compatibility launcher, then runs
the packaged runtime's auth classifier for real, clean, and synthetic auth
profiles. It proves the packaged runtime can guide a new user to
`CODEX_HOME=<clean-codex-home> <clean-home>/.local/bin/omnigent-stock-codex-compat login`
without printing credential material. It does not automate browser/device
login, validate token freshness with OpenAI, or copy credentials to another
machine.
...
Current installed-runtime scheduled updater status on 2026-07-08:
`--proof stock-codex-compat-pkg-update-agent --codex-path
/Users/joshuakaunert/.local/omnigent/codex-stock/0.142.5/codex` passed. The
proof staged the package payload under a temporary installed root, validated
that `update_stock_codex_compat.py` is packaged and recorded in manifests, wrote
a user LaunchAgent plist without loading launchd state, and verified the
scheduled `ProgramArguments` delegate to `uvx --from <installed-runtime> python
<installed-runtime>/scripts/update_stock_codex_compat.py`. It verified the plist
follows the current launcher manifest rather than baking a proof-only
`--current-codex` path, then ran the updater directly to stage, promote, and
no-op check the stable official stock-Codex target. The live run selected cask
version `0.142.5`, release URL
`https://github.com/openai/codex/releases/download/rust-v0.142.5/codex-aarch64-apple-darwin.tar.gz`,
cask SHA-256 `7156b19962735c9cfb555cdd7babe8c40e7976881f8712b781199219d2e3a707`,
package SHA-256 `4fca06b42360d093569b4aabc6e521c96e2b99191cc3beb562bece5fef90702b`,
and source-bundle SHA-256
`13ceb620d290294576b8da28859e2545d2d16b380b9ea16ec2caade34d561a99`.
This closes packaged updater scheduling shape while leaving final launchd
enablement policy, pre-release channel policy, and independent archive signature
policy as separate product choices.
...
credential material was not printed. The proof now also installs/selects the
compatibility launcher from the installed runtime and emits guided login as
`CODEX_HOME=<clean-codex-home> <clean-home>/.local/bin/omnigent-stock-codex-compat login`.
This closes clean auth classification and launcher-owned guided onboarding from
...
`7256d678-f5cc-4676-ba35-4d3db4b28820`, reported final stapled package SHA-256
`7ec3bec64157f1bed9f56ad1e457ef556912ca3ddd1ac95db5b534066bdf2f3d`, included
the packaged first-run bootstrap payloads, stapled successfully, validated the
staple successfully, and `spctl -a -vv -t install` accepted the package as
...
9. `stock-codex-compat` bridge proof
   ...
   - Remaining product decision: decide whether compatibility mode may require
     ...
     `auth.json` reports available, keeps credential material out of
     classifier output, and points guided login at the installed compatibility
     launcher instead of the raw stock Codex binary.
     ...
     The clean-VM remote acquisition gate is green too:
     `stock-codex-compat-pkg-clean-vm-remote-acquisition` installs the
     signed/notarized package into a disposable Tart VM's real `/Library`, runs
     the packaged bootstrapper with explicit remote-download opt-in and the
     official OpenAI GitHub release channel policy, downloads and verifies stock
     Codex inside the VM, proves no host stock Codex binary was uploaded,
     bootstraps the compatibility launcher, classifies clean auth, and removes
     user/package state.
     ...
     scope requires it, plus launchd enablement policy or independent archive
     signature policy if product scope requires them.
