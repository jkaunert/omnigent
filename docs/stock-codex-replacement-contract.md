...
| Signed/notarized stock Codex compatibility pkg external clean-user gate | `blocked` until an operator supplies a marked disposable user home or clean VM profile | `scripts/prove_stock_codex_replacement.py --proof stock-codex-compat-pkg-external-clean-user --codex-path /Users/joshuakaunert/.local/omnigent/codex-stock/0.142.5/codex --pkg-path <artifact.pkg> --clean-user-home <home>` consumes the same signed/notarized package artifact and requires `<home>/.omnigent-stock-codex-compat-clean-user-ok` before it will mutate the supplied profile. It refuses homes with preexisting compatibility launcher, manifest, adapter package, Codex stock cache, clean auth canary, or in-flight proof state. When prerequisites are present, the gate installs the signed package onto a temporary mounted target volume, validates receipt metadata and required runtime payload files, provisions stock Codex into the marked profile's default cache from the installed runtime, installs the default adapter package, installs and selects `~/.local/bin/omnigent-stock-codex-compat`, verifies `--version` plus launcher probe delegation, runs non-mutating doctor, classifies canary `CODEX_HOME` as `needs-auth`, executes the generated rollback command, removes canary-created user state while preserving the operator marker, removes package payload and receipt state from the temporary target volume, and detaches the image. Unit coverage in `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_external_clean_user_requires_marked_home` and `tests/scripts/test_prove_stock_codex_replacement.py::test_stock_codex_compat_pkg_external_clean_user_uses_marked_home` covers marker enforcement, prebuilt package validation, mounted-target install, external-home provisioning, launcher bootstrap, auth classification, rollback, user-state cleanup, receipt cleanup, and detach with fake tools. This creates the clean-user/VM validation surface without silently creating macOS users or modifying a real `/Library`; it remains blocked as production evidence until run against an actual marked throwaway macOS user profile or separate clean VM. |
...
instead of this host's real `/Library`. The current code and tests prove the
package install, receipt validation, external-home provisioning, launcher
bootstrap, clean auth classification, rollback, user-state cleanup, package
cleanup, and detach sequence with fake tools. The release claim remains blocked
until the gate is run against an actual marked throwaway macOS account or clean
VM profile.
...
9. `stock-codex-compat` bridge proof
   ...
   - Remaining product decision: decide whether compatibility mode may require
     ...
     implemented but intentionally blocked until pointed at an operator-marked
     disposable macOS home or clean VM profile; it refuses unmarked or already
     populated profiles and does not create users itself. The production
