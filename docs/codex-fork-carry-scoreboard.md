...
The current blocker is not more simulator gesture coverage or router-selection
parity. The runtime diagnostic and remote-resume groups need a product-scope
decision before they can be scored honestly.

## Next Proof Gate

The next narrow gate should address the runtime diagnostic carry group. Decide
whether stock-Codex cutover needs replacement behavior for:

- provider stream lifecycle diagnostics
- early output-item stall classification
- HTTP fallback guarding by provider URL

If those remain product requirements, build an Omnigent-side session or stream
diagnostic proof instead of carrying the behavior in a Codex fork.
