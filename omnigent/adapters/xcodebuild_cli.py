...
XCODEBUILD_CLI_TYPE_TEXT_TOOL_NAME = "xcodebuildmcp_simulator_type_text"
XCODEBUILD_CLI_TAP_TOOL_NAME = "xcodebuildmcp_simulator_tap"
...
XCODEBUILDMCP_CLI_TYPE_TEXT_COMMAND = ("xcodebuildmcp", "ui-automation", "type-text")
XCODEBUILDMCP_CLI_TAP_COMMAND = ("xcodebuildmcp", "ui-automation", "tap")
XCODEBUILDMCP_CLI_WAIT_FOR_UI_COMMAND = ("xcodebuildmcp", "ui-automation", "wait-for-ui")
XCRUN_SIMCTL_COMMAND = ("xcrun", "simctl")
...
class XcodeBuildCliAdapterPolicy:
    ...
    type_text_tool_name: str = XCODEBUILD_CLI_TYPE_TEXT_TOOL_NAME
    tap_tool_name: str = XCODEBUILD_CLI_TAP_TOOL_NAME
    ...
    type_text_command_prefix: tuple[str, ...] = XCODEBUILDMCP_CLI_TYPE_TEXT_COMMAND
    tap_command_prefix: tuple[str, ...] = XCODEBUILDMCP_CLI_TAP_COMMAND
    wait_for_ui_command_prefix: tuple[str, ...] = XCODEBUILDMCP_CLI_WAIT_FOR_UI_COMMAND
    simctl_command_prefix: tuple[str, ...] = XCRUN_SIMCTL_COMMAND
...
def build_xcodebuildmcp_simulator_type_text_tool_source(
    policy: XcodeBuildCliAdapterPolicy = DEFAULT_XCODEBUILD_CLI_POLICY,
) -> str:
    """Build the self-contained Python source for the generated type-text tool."""
    if not policy.type_text_tool_name.isidentifier():
        raise ValueError(
            "type_text_tool_name must be a valid Python identifier: "
            f"{policy.type_text_tool_name!r}"
        )
    if policy.timeout_seconds <= 0:
        raise ValueError("timeout_seconds must be greater than zero")
    build_run_command_prefix = list(policy.command_prefix)
    snapshot_ui_command_prefix = list(policy.snapshot_ui_command_prefix)
    type_text_command_prefix = list(policy.type_text_command_prefix)
    wait_for_ui_command_prefix = list(policy.wait_for_ui_command_prefix)
    env_overrides = dict(policy.env_overrides)
    axe_path = policy.axe_path
    axe_path_env_var = policy.axe_path_env_var
    target_axe_path_env_var = policy.target_axe_path_env_var
    ...
    return textwrap.dedent(
        f'''\
        """XcodeBuildMCP simulator type-text CLI adapter generated by Omnigent."""
        ...
        _BUILD_RUN_COMMAND_PREFIX = {build_run_command_prefix!r}
        _SNAPSHOT_UI_COMMAND_PREFIX = {snapshot_ui_command_prefix!r}
        _TYPE_TEXT_COMMAND_PREFIX = {type_text_command_prefix!r}
        _WAIT_FOR_UI_COMMAND_PREFIX = {wait_for_ui_command_prefix!r}
        _ENV_OVERRIDES = {env_overrides!r}
        _ALLOWED_SIMULATOR_PREFIXES = {allowed_simulator_prefixes!r}
        _ALLOWED_DERIVED_DATA_ROOTS = {allowed_derived_data_roots!r}
        _ALLOWED_EXTRA_ARGS = {allowed_extra_args!r}
        ...
        def _validation_error(
            ...
            if "\\n" in text or "\\r" in text:
                return "Error: text must be a single line."
            if len(text) > 200:
                return "Error: text must be 200 characters or fewer."
            normalized_extra_args = list(extra_args or [])
            unexpected_extra_args = [
                arg for arg in normalized_extra_args if arg not in _ALLOWED_EXTRA_ARGS
            ]
        ...
        def _runtime_snapshot(
            ...
            data = _require_dict(result.get("data"), "data")
            if isinstance(data, str):
                return data
            summary = _require_dict(data.get("summary"), "data.summary")
            if isinstance(summary, str):
                return summary
            capture_value = data.get("capture")
            if capture_value is None and data.get("type") == "runtime-snapshot":
                capture_value = data
            capture = _require_dict(capture_value, "data.capture")
            if isinstance(capture, str):
                return capture
            if summary.get("status") != "SUCCEEDED":
                return f"Error: {{label}} status was {{summary.get('status')!r}}."
            if capture.get("type") != "runtime-snapshot":
                return f"Error: {{label}} capture type was {{capture.get('type')!r}}."
            count = capture.get("count")
            targets = capture.get("targets")
            if not isinstance(count, int) or count <= 0:
                return f"Error: {{label}} count was not positive: {{count!r}}."
            if not isinstance(targets, list) or not targets:
                return f"Error: {{label}} result did not include targets."
            return {{"summary": summary, "capture": capture, "count": count, "targets": targets}}


        def _selected_type_text_target(targets: list[object]) -> dict[str, str] | str:
            for target in targets:
                ...
                if len(parts) < 3:
                    continue
                actions = set(parts[1].split(","))
                if parts[0] and "typeText" in actions and parts[2] == "text-field":
                    return {{"elementRef": parts[0], "target": target}}
            return "Error: snapshot-ui result did not include a typeText text-field target."


        @tool
        def {policy.type_text_tool_name}(
            project_path: str,
            scheme: str,
            configuration: str,
            simulator_name: str,
            ...
            """Build, launch, type text into the first text field, and verify it."""
            validation_error = _validation_error(
                project_path,
                scheme,
                configuration,
            ...
            try:
                ...
                if error is not None:
                    return error
                assert snapshot_result is not None
                before_snapshot = _runtime_snapshot(snapshot_result, "snapshot-ui")
                if isinstance(before_snapshot, str):
                    return before_snapshot
                selected = _selected_type_text_target(before_snapshot["targets"])
                if isinstance(selected, str):
                    return selected
                element_ref = selected["elementRef"]

                type_command = [
                    *_with_socket(_TYPE_TEXT_COMMAND_PREFIX, socket_path),
                    "--simulator-id",
                    simulator_id,
                    "--element-ref",
                    element_ref,
                ...
                type_result, error = _run_json_command(type_command, env)
                if error is not None:
                    return error
                assert type_result is not None
                type_data = _require_dict(type_result.get("data"), "data")
                if isinstance(type_data, str):
                    return type_data
                type_summary = _require_dict(type_data.get("summary"), "data.summary")
                if isinstance(type_summary, str):
                    return type_summary
                if type_summary.get("status") != "SUCCEEDED":
                    return f"Error: type-text status was {{type_summary.get('status')!r}}."

                wait_command = [
                    *_with_socket(_WAIT_FOR_UI_COMMAND_PREFIX, socket_path),
                    "--simulator-id",
                    simulator_id,
                    "--predicate",
                    "textContains",
                ...
                wait_result, error = _run_json_command(wait_command, env)
                if error is not None:
                    return error
                assert wait_result is not None
                after_snapshot = _runtime_snapshot(wait_result, "wait-for-ui")
                if isinstance(after_snapshot, str):
                    return after_snapshot
                after_targets = after_snapshot["targets"]
                if not any(text in str(target) for target in after_targets):
                    return "Error: wait-for-ui result did not include the typed text."

                before_capture = before_snapshot["capture"]
                after_capture = after_snapshot["capture"]
                return json.dumps(
                    {{
                        "buildStatus": build_summary.get("status"),
                        "typeTextStatus": type_summary.get("status"),
                        "waitStatus": after_snapshot["summary"].get("status"),
                        "bundleId": build_artifacts.get("bundleId"),
                        "simulatorId": simulator_id,
                        "elementRef": element_ref,
                        "typedText": text,
                        "verifiedText": text,
                        "beforeTarget": selected["target"],
                        "beforeScreenHash": before_capture.get("screenHash"),
                        "afterScreenHash": after_capture.get("screenHash"),
                        "afterCount": after_snapshot["count"],
                        "afterTargets": after_targets[:12],
                    }},
                    indent=2,
                    sort_keys=True,
                )
...
def build_xcodebuildmcp_simulator_tap_tool_source(
    policy: XcodeBuildCliAdapterPolicy = DEFAULT_XCODEBUILD_CLI_POLICY,
) -> str:
    """Build the self-contained Python source for the generated tap tool."""
    if not policy.tap_tool_name.isidentifier():
        raise ValueError(
            f"tap_tool_name must be a valid Python identifier: {policy.tap_tool_name!r}"
        )
    if policy.timeout_seconds <= 0:
        raise ValueError("timeout_seconds must be greater than zero")
    build_run_command_prefix = list(policy.command_prefix)
    snapshot_ui_command_prefix = list(policy.snapshot_ui_command_prefix)
    type_text_command_prefix = list(policy.type_text_command_prefix)
    tap_command_prefix = list(policy.tap_command_prefix)
    wait_for_ui_command_prefix = list(policy.wait_for_ui_command_prefix)
    simctl_command_prefix = list(policy.simctl_command_prefix)
    env_overrides = dict(policy.env_overrides)
    axe_path = policy.axe_path
    axe_path_env_var = policy.axe_path_env_var
    target_axe_path_env_var = policy.target_axe_path_env_var
    ...
    return textwrap.dedent(
        f'''\
        """XcodeBuildMCP simulator tap CLI adapter generated by Omnigent."""
        ...
        _SNAPSHOT_UI_COMMAND_PREFIX = {snapshot_ui_command_prefix!r}
        _TYPE_TEXT_COMMAND_PREFIX = {type_text_command_prefix!r}
        _TAP_COMMAND_PREFIX = {tap_command_prefix!r}
        _WAIT_FOR_UI_COMMAND_PREFIX = {wait_for_ui_command_prefix!r}
        _SIMCTL_COMMAND_PREFIX = {simctl_command_prefix!r}
        _ENV_OVERRIDES = {env_overrides!r}
        _ALLOWED_SIMULATOR_PREFIXES = {allowed_simulator_prefixes!r}
        _ALLOWED_DERIVED_DATA_ROOTS = {allowed_derived_data_roots!r}
        _ALLOWED_EXTRA_ARGS = {allowed_extra_args!r}
        ...
        def _uninstall_app(
            ...
            *,
            ...
            if not simulator_id or not bundle_id:
                return "Error: simulator id and bundle id are required for app reset."
            try:
                completed = subprocess.run(
                    [*_SIMCTL_COMMAND_PREFIX, "uninstall", simulator_id, bundle_id],
                    check=False,
                    capture_output=True,
                    text=True,
                    timeout=30,
                ...
                if required:
                    return "Error: simctl uninstall timed out after 30 seconds."
                return None
            except Exception as exc:
                if required:
                    return f"Error: simctl uninstall failed: {{exc}}."
                return None
            if required and completed.returncode != 0:
                detail = (completed.stderr or completed.stdout).strip()
                return (
                    f"Error: simctl uninstall exited {{completed.returncode}}: "
                    f"{{detail[:1000]}}"
                )
            return None
        ...
        def _target_record(target: object) -> dict[str, str] | None:
            if not isinstance(target, str):
                return None
            parts = target.split("|")
            if len(parts) < 3:
        ...
        def _selected_type_text_target(targets: list[object]) -> dict[str, str] | str:
            for target in targets:
                record = _target_record(target)
                if record is None:
                    continue
                actions = set(record["actions"].split(","))
                if (
                    record["elementRef"]
                    and "typeText" in actions
                    and record["role"] == "text-field"
                ):
                    return record
            return "Error: snapshot-ui result did not include a typeText text-field target."


        def _selected_tap_button_target(targets: list[object], label: str) -> dict[str, str] | str:
            for target in targets:
                record = _target_record(target)
                if record is None:
                    continue
                ...
                if (
                    record["elementRef"]
                    and "tap" in actions
                    and record["role"] == "button"
                    and record["label"] == label
                ):
                    return record
            return f"Error: snapshot-ui result did not include a tappable {{label!r}} button."


        def _selected_text_value_target(targets: list[object], value: str) -> dict[str, str] | str:
            for target in targets:
                record = _target_record(target)
                if record is None:
                    continue
                ...
                if (
                    ...
                    return record
            return f"Error: settled UI did not include expected text-field value {{value!r}}."


        def _origin_text(text: str) -> str:
        ...
        def _build_and_run(
            ...
            if build_summary.get("status") != "SUCCEEDED":
                return f"Error: build-and-run status was {{build_summary.get('status')!r}}."
            simulator_id = build_artifacts.get("simulatorId")
            bundle_id = build_artifacts.get("bundleId")
            if not isinstance(simulator_id, str) or not simulator_id:
                return "Error: build-and-run result did not include simulatorId."
            if not isinstance(bundle_id, str) or not bundle_id:
                return "Error: build-and-run result did not include bundleId."
            return {{"summary": build_summary, "artifacts": build_artifacts}}


        @tool
        def {policy.tap_tool_name}(
            project_path: str,
            scheme: str,
            configuration: str,
            simulator_name: str,
            ...
            """Reset app state, type into the setup field, tap Connect, and verify state."""
            validation_error = _validation_error(
                project_path,
                scheme,
                configuration,
            ...
            bundle_id = ""
            try:
                pre_reset_build = _build_and_run(build_payload, socket_path, env)
                if isinstance(pre_reset_build, str):
                    return pre_reset_build
                pre_reset_artifacts = pre_reset_build["artifacts"]
                simulator_id = str(pre_reset_artifacts.get("simulatorId", ""))
                bundle_id = str(pre_reset_artifacts.get("bundleId", ""))
                reset_error = _uninstall_app(simulator_id, bundle_id, env, required=True)
                if reset_error is not None:
                    return reset_error

                build = _build_and_run(build_payload, socket_path, env)
                if isinstance(build, str):
                    return build
                build_summary = build["summary"]
                build_artifacts = build["artifacts"]
                simulator_id = str(build_artifacts.get("simulatorId", simulator_id))
                bundle_id = str(build_artifacts.get("bundleId", bundle_id))
                ...
                if isinstance(before_snapshot, str):
                    return before_snapshot
                text_target = _selected_type_text_target(before_snapshot["targets"])
                ...
                typed_wait_result, error = _run_json_command(typed_wait_command, env)
                if error is not None:
                    return error
                assert typed_wait_result is not None
                typed_snapshot = _runtime_snapshot(typed_wait_result, "typed wait-for-ui")
                if isinstance(typed_snapshot, str):
                    return typed_snapshot
                typed_targets = typed_snapshot["targets"]
                typed_text_target = _selected_text_value_target(typed_targets, text)
                if isinstance(typed_text_target, str):
                    return typed_text_target
                tap_target = _selected_tap_button_target(typed_targets, "Connect")
                if isinstance(tap_target, str):
                    return tap_target

                tap_command = [
                    *_with_socket(_TAP_COMMAND_PREFIX, socket_path),
                    "--simulator-id",
                    simulator_id,
                    "--element-ref",
                    tap_target["elementRef"],
                    "--post-delay",
                    "1",
                    "--output",
                    "json",
                ]
                tap_result, error = _run_json_command(tap_command, env)
                if error is not None:
                    return error
                assert tap_result is not None
                tap_data = _require_dict(tap_result.get("data"), "data")
                if isinstance(tap_data, str):
                    return tap_data
                tap_summary = _require_dict(tap_data.get("summary"), "data.summary")
                if isinstance(tap_summary, str):
                    return tap_summary
                if tap_summary.get("status") != "SUCCEEDED":
                    return f"Error: tap status was {{tap_summary.get('status')!r}}."
                ...
                settled_result, error = _run_json_command(settled_command, env)
                if error is not None:
                    return error
                assert settled_result is not None
                after_snapshot = _runtime_snapshot(settled_result, "settled wait-for-ui")
                if isinstance(after_snapshot, str):
                    return after_snapshot
                after_targets = after_snapshot["targets"]
                post_tap_text = _origin_text(text)
                after_text_target = _selected_text_value_target(after_targets, post_tap_text)
                if isinstance(after_text_target, str):
                    return after_text_target
                if any(text in str(target) for target in after_targets):
                    return "Error: settled UI still included the pre-tap proof URL."

                typed_capture = typed_snapshot["capture"]
                after_capture = after_snapshot["capture"]
                return json.dumps(
                    {{
                        "preResetBuildStatus": pre_reset_build["summary"].get("status"),
                        "resetStatus": "SUCCEEDED",
                        "buildStatus": build_summary.get("status"),
                        "typeTextStatus": type_summary.get("status"),
                        "typedWaitStatus": typed_snapshot["summary"].get("status"),
                        "tapStatus": tap_summary.get("status"),
                        "settledStatus": after_snapshot["summary"].get("status"),
                        "bundleId": bundle_id,
                        "simulatorId": simulator_id,
                        "textElementRef": text_target["elementRef"],
                        "tapElementRef": tap_target["elementRef"],
                        "typedText": text,
                        "postTapText": post_tap_text,
                        "typedTarget": typed_text_target["target"],
                        "tapTarget": tap_target["target"],
                        "afterTapTarget": after_text_target["target"],
                        "typedScreenHash": typed_capture.get("screenHash"),
                        "afterTapScreenHash": after_capture.get("screenHash"),
                        "afterTapCount": after_snapshot["count"],
                        "afterTapTargets": after_targets[:12],
                    }},
                    indent=2,
                    sort_keys=True,
                )
            finally:
                if simulator_id and bundle_id:
                    _uninstall_app(simulator_id, bundle_id, env, required=False)
                _stop_daemon(socket_path, env)
                shutil.rmtree(socket_dir, ignore_errors=True)
        '''
    )
...
def write_xcodebuildmcp_simulator_type_text_tool(
    agent_dir: Path,
    *,
    policy: XcodeBuildCliAdapterPolicy = DEFAULT_XCODEBUILD_CLI_POLICY,
) -> Path:
    """Write the generated XcodeBuildMCP CLI type-text tool into an agent bundle."""
    source = build_xcodebuildmcp_simulator_type_text_tool_source(policy)
    tools_dir = agent_dir / "tools" / "python"
    tools_dir.mkdir(parents=True, exist_ok=True)
    tool_path = tools_dir / f"{policy.type_text_tool_name}.py"
    tool_path.write_text(source, encoding="utf-8")
    return tool_path


def write_xcodebuildmcp_simulator_tap_tool(
    agent_dir: Path,
    *,
    policy: XcodeBuildCliAdapterPolicy = DEFAULT_XCODEBUILD_CLI_POLICY,
) -> Path:
    """Write the generated XcodeBuildMCP CLI tap tool into an agent bundle."""
    source = build_xcodebuildmcp_simulator_tap_tool_source(policy)
    tools_dir = agent_dir / "tools" / "python"
    tools_dir.mkdir(parents=True, exist_ok=True)
    tool_path = tools_dir / f"{policy.tap_tool_name}.py"
    tool_path.write_text(source, encoding="utf-8")
    return tool_path
