...
def test_stock_codex_bridge_adapter_spec_uses_closed_test_schema(
    tmp_path: Path,
) -> None:
    spec = build_xcodebuildmcp_simulator_test_stock_codex_bridge_adapter_spec()

    manifest = build_stock_codex_compat_adapter_manifest(tmp_path / "bin", (spec,))

    assert manifest["tools"][0]["name"] == XCODEBUILD_CLI_TEST_TOOL_NAME
    assert manifest["tools"][0]["capability"] == "xcodebuildmcp-simulator-test"
    assert manifest["tools"][0]["parameters"]["required"] == [
        "project_path",
        "scheme",
        "configuration",
...
def test_stock_codex_bridge_handler_runs_simulator_test_with_adapter_env(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    project_path = tmp_path / "Demo.xcodeproj"
    ...
    fake_xcodebuildmcp.chmod(0o755)
    policy = XcodeBuildCliAdapterPolicy(
        test_command_prefix=(str(fake_xcodebuildmcp), "simulator", "test"),
        allowed_derived_data_roots=(str(tmp_path),),
        timeout_seconds=5,
    )
    handler = build_xcodebuildmcp_simulator_test_stock_codex_bridge_handler(policy)
    monkeypatch.setenv(XCODEBUILDMCP_AXE_PATH_ENV, "/ambient/axe")
    ...
    assert response.status == "ok"
    assert response.exitCode == 0
    assert "tests passed" in response.stdout
...
def test_generated_type_text_tool_source_names_expected_tool() -> None:
    source = build_xcodebuildmcp_simulator_type_text_tool_source()

    assert f"def {XCODEBUILD_CLI_TYPE_TEXT_TOOL_NAME}(" in source
    assert "xcodebuildmcp" in source
    assert "snapshot-ui" in source
...
def test_generated_type_text_tool_launches_then_types_and_verifies(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    project_path = tmp_path / "Demo.xcodeproj"
    ...
    policy = XcodeBuildCliAdapterPolicy(allowed_derived_data_roots=(str(tmp_path),))
    tool_path = write_xcodebuildmcp_simulator_type_text_tool(tmp_path, policy=policy)
    module = _load_module(tool_path)
    calls: list[tuple[list[str], dict[str, Any]]] = []
    ...
    invalid_result = module.xcodebuildmcp_simulator_type_text(
        str(project_path),
        "Demo",
        "Debug",
        "Vision Pro",
    ...
    valid_result = module.xcodebuildmcp_simulator_type_text(
        str(project_path),
        "Demo",
        "Debug",
        "iPhone 17",
    ...
    build_cmd, build_kwargs = calls[0]
    snapshot_cmd, snapshot_kwargs = calls[1]
    type_cmd, type_kwargs = calls[2]
    wait_cmd, wait_kwargs = calls[3]
    stop_cmd, stop_kwargs = calls[4]
    assert build_cmd[:5] == [
        "xcodebuildmcp",
        "--socket",
        build_cmd[2],
    ...
    assert snapshot_kwargs == build_kwargs
    assert type_kwargs == build_kwargs
    assert wait_kwargs == build_kwargs
    assert stop_kwargs == {
        "check": False,
        "capture_output": True,
        "text": True,
    ...
    assert result == {
        "afterCount": 2,
        "afterScreenHash": "after",
        "afterTargets": [
            f"e14|typeText|text-field||{proof_text}|",
            "e15|tap|button|Connect||",
        ],
        "beforeScreenHash": "before",
        "beforeTarget": "e14|typeText|text-field||http://localhost:6767|",
        "buildStatus": "SUCCEEDED",
        "bundleId": "ai.omnigent.ios",
        "elementRef": "e14",
        "simulatorId": "SIM-123",
